...
/// The five layout presets a layout hotkey can select (resolved to panels in M2c-3).
#[derive(Serialize, Deserialize, Clone, Copy, Debug, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum LayoutId { Screen, Camera, Presenter, ScreenOnly, CameraOnly }

/// A hotkey-driven action on the capture clock. `SetLayout` is a persistent
...
mod tests {
    ...
    fn round_trips_actions_json() {
        let log = ActionLog { actions: vec![
            ActionEvent { t: 100, kind: ActionKind::SetLayout(LayoutId::Camera) },
            ActionEvent { t: 250, kind: ActionKind::ZoomHoldStart },
            ActionEvent { t: 900, kind: ActionKind::ZoomHoldEnd },
        ]};
        ...
        assert_eq!(back.actions[0].kind, ActionKind::SetLayout(LayoutId::Camera));
