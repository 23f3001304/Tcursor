...
const target = (over: Partial<TargetLike> = {}): TargetLike => ({ tagName: "DIV", role: null, isContentEditable: false, ...over });
const key = (over: Partial<{ key: string; ctrlKey: boolean; metaKey: boolean; altKey: boolean; repeat: boolean }> = {}) =>
  ({ key: "z", ctrlKey: false, metaKey: false, altKey: false, repeat: false, ...over });
const ctx = (over: Partial<{ hasSel: boolean; modalOpen: boolean; shortcutsOpen: boolean; target: TargetLike }> = {}) =>
...
describe("ownsSpace", () => {
  ...
  // Review round 1 minor: `role="slider"` does NOT own Space per WAI-ARIA (only arrow keys move
  // a slider); Slider.tsx doesn't handle it either. Previously including it here made Space a
  // DEAD key on a focused Slider - resolveKeyAction withheld "play" AND nothing else fired it.
...
describe("resolveKeyAction (the full decision useEditorKeymap acts on)", () => {
  ...
  // Review round 1 minor: a focused Slider (role=slider) no longer owns Space, so it must fall
  // through to the normal "play" action instead of being silently swallowed.
  ...
  // Review round 1 minor: "?" toggles ShortcutsOverlay, and the wiring is a toggle (open<->closed)
  // - so it must stay reachable even while `modalOpen` is true, PROVIDED shortcuts is the reason.
  describe("? special-cased ahead of the modal bail", () => {
    ...
    it("still fires (to close it) while ShortcutsOverlay is the open modal", () => {
      expect(resolveKeyAction(key({ key: "?" }), ctx({ modalOpen: true, shortcutsOpen: true }))).toBe("overlay");
    });
    ...
    it("is still blocked by typing and still respects its own repeat guard", () => {
      expect(resolveKeyAction(key({ key: "?" }), ctx({ shortcutsOpen: true, target: target({ tagName: "INPUT" }) }))).toBeNull();
