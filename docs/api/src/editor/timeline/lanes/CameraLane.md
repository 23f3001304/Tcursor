# src/editor/timeline/lanes/CameraLane.tsx
...
Renders an `.e-camlane` wrapper (which used to be what let the curve popover escape the row's `overflow:hidden`; the popover is portalled out of the whole lane now, but the wrapper stays as the lane's own positioning box) around a single `.e-camrow`: the dashed `.e-cambase` baseline, the `.e-camspan` ownership bar behind everything, one `.e-camseg` per consecutive keyframe pair, and every `.e-camkf` diamond on top (no layer-stacking - keyframes are points, not overlapping regions).
...
- `hasWebcam: boolean` - `hasWebcamSignal(layout)` (`../model/editorData.ts`), threaded from `Editor` through `Timeline`. Gates the empty-lane hint below off for a recording with no webcam (sweep-2 gate finding: it invited keyframing a panel that was never going to show anything).
...
**Span bar (Task 27).** `.e-camspan` spans `[first - KF_BLEND_MS, last + KF_BLEND_MS]` of the DRAFT-adjusted keyframe times (so it follows a diamond mid-drag), rendered in the lane's `--e-cam` colour at 12%. That fill is the module const `CAM_12` = `color-mix(in srgb, var(--e-cam) 12%, transparent)`, not a literal pink, so the bar tracks whichever camera accent the active theme declares - the same way every other camera-coloured mark in this lane does. Its `background` is set inline rather than in CSS because the two fade ramps must be exactly `KF_BLEND_MS` wide, which as a percentage depends on the span's own length: `linear-gradient(90deg, transparent 0%, CAM_12 R%, CAM_12 (100-R)%, transparent 100%)` with `R = KF_BLEND_MS / spanLength`, each stop clamped so a short span (a single keyframe, where the whole bar is the two blend windows) degrades into a symmetric triangle instead of inverting. Zero keyframes -> no bar at all. This is a STATIC fill: plain CSS, no Motion, because nothing about it animates - only its endpoints change, and those change because the underlying data did.
...
**Mount/hover.** Each diamond fades/scales in on mount (no hover growth since 2026-09-13, same ruling as the pills), but the drag itself is a manual `pointermove`/`pointerup` window-listener pair - not native HTML5 drag-and-drop and not Motion's own drag gesture.
...
**Transition curve.** Clicking a `.e-camseg` opens `CameraCurvePop` (`CameraCurvePop.md`), a popover of the five motion presets (`PresetRow`, M3) over the segment; picking one applies `update_camera_move { id, easing }` to the later keyframe (easing eases *into* it) and keeps the popover open so curves can be auditioned against the live preview. It dismisses on Escape or an outside pointerdown (segments excepted, so clicking a neighbour re-anchors instead of closing), and on a scroll or resize that moves the lane out from under it.

The `pick` state carries the clicked segment's **viewport rect**, captured from `e.currentTarget.getBoundingClientRect()` at pointerdown, rather than the midpoint percentage it used to hold: the popover is portalled out of the lane now (width/clipping audit, 2026-09-14 - `.e-tracks` was cutting it off on both axes), so it places from a real box. `closePick` is `useCallback`'d because the popover registers scroll/resize listeners keyed on its identity.

**`.e-campop`'s own `onPointerDown` stopPropagation (bug-sweep-2 Task 8, M3).** Every other draggable/clickable element in this lane calls `e.stopPropagation()` on its own `pointerdown` (the pattern `Timeline.tsx`'s track-body scrub handler relies on to tell "a child claimed this pointer" from "a genuine body scrub"). The popover wrapper didn't, so a click on a curve preset bubbled all the way to `.e-tlbody`, which seeks the playhead, pauses playback, AND takes pointer capture - stealing the button's own `onClick` (the easing choice) about half the time too, since the subsequent pointerup/click retargeted to `.e-tlbody` instead of the button. The wrapper `div` still stops propagation on its own `pointerdown`, same as every sibling in this file - and it still has to, because React events propagate through the React tree rather than the DOM one, so portalling it did not move them out of reach.
