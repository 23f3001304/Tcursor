...
fn empty_track_samples_to_none() {
    ...
    assert_eq!(track.sample(5000), None);
}
...
fn single_keyframe_holds_at_any_time() {
    ...
    assert_eq!(track.sample(0), Some(want));
    assert_eq!(track.sample(1000), Some(want));
    assert_eq!(track.sample(50_000), Some(want));
}
...
fn holds_first_pose_before_and_at_first_keyframe() {
    ...
    assert_eq!(track.sample(0), Some(first));
    assert_eq!(track.sample(999), Some(first));
    assert_eq!(track.sample(1000), Some(first));
}
...
fn holds_last_pose_at_and_after_last_keyframe() {
    ...
    assert_eq!(track.sample(2000), Some(last));
    assert_eq!(track.sample(9000), Some(last));
}
...
fn coincident_keyframe_times_snap_to_b_without_dividing_by_zero() {
    ...
    let track = CameraMoveTrack::from_doc(&moves);
    let p = track.sample(1000).unwrap();
...
fn out_of_order_input_is_sorted_defensively() {
    ...
    let track = CameraMoveTrack::from_doc(&moves);
    assert_eq!(track.sample(0), Some(CamPose { x: 0.0, y: 0.0, size: 0.0 }));
    assert_eq!(track.sample(2000), Some(CamPose { x: 0.9, y: 0.9, size: 0.5 }));
    let mid = track.sample(1000).unwrap();
