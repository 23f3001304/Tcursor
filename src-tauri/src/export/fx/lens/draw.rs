...
use crate::export::fx::lens::{BackLens, CursorLens, Lenses, ZOOM};
...
fn over(out: &mut [u8], i: usize, c: [f32; 3], a: f32) {
    ...
    if a <= 0.002 || i + 3 >= out.len() {
        return;
    }
    for (k, ch) in [c[2], c[1], c[0]].into_iter().enumerate() {
        out[i + k] = (ch * a + out[i + k] as f32 * (1.0 - a))
            .round()
...
fn darken(out: &mut [u8], i: usize, f: f32) {
    ...
    for k in 0..3 {
        out[i + k] = (out[i + k] as f32 * f.clamp(0.0, 1.0)) as u8;
    }
}
...
fn for_box(ow: u32, oh: u32, b: [f32; 4], mut f: impl FnMut(usize, f32, f32)) {
    ...
    for y in y0..y1 {
        for x in x0..x1 {
            f(
                ((y as u32 * ow + x as u32) * 4) as usize,
                x as f32 + 0.5,
                y as f32 + 0.5,
            );
        }
...
fn rr_sd(px: f32, py: f32, mn: [f32; 2], mx: [f32; 2], r: f32) -> f32 {
    ...
    let (qx, qy) = (
        (px - cx).abs() - ((mx[0] - mn[0]) * 0.5 - r),
        (py - cy).abs() - ((mx[1] - mn[1]) * 0.5 - r),
    );
...
fn ink_cov(d: f32, p: f32, reach: f32) -> f32 {
    ...
    let r = (clickfx::ease_out(p) * reach).max(0.5);
    (1.0 - clickfx::smoothstep(r * 0.55, r, d)) * (1.0 - p.clamp(0.0, 1.0))
}
...
fn snapshot(out: &[u8], ow: u32, oh: u32, b: [f32; 4]) -> Source {
    ...
    let (x1, y1) = (
        (b[2].ceil() as i32).min(ow as i32),
        (b[3].ceil() as i32).min(oh as i32),
    );
...
fn magnify(src: &Source, c: [f32; 2], px: f32, py: f32) -> [f32; 3] {
    let (sx, sy) = (
        c[0] + (px - c[0]) / ZOOM - src.x0 as f32 - 0.5,
        c[1] + (py - c[1]) / ZOOM - src.y0 as f32 - 0.5,
    );
    ...
    let (a, b, d, e) = (
        tap(fx as i32, fy as i32),
        tap(fx as i32 + 1, fy as i32),
        tap(fx as i32, fy as i32 + 1),
        tap(fx as i32 + 1, fy as i32 + 1),
    );
...
fn draw_back(out: &mut [u8], ow: u32, oh: u32, b: &BackLens, accent: [u8; 3]) {
    ...
    let half = [
        (b.mx[0] - b.mn[0]) * 0.5 * b.squash,
        (b.mx[1] - b.mn[1]) * 0.5 * b.squash,
    ];
    let (mn, mx) = (
    ...
    for_box(ow, oh, bx, |i, px, py| {
        ...
        if b.ring {
            ...
            over(
                ...
                i,
                acc,
                INK * ink_cov((px - b.ink_at[0]).hypot(py - b.ink_at[1]), b.ink, reach) * m,
            );
        }
    });
}
...
fn draw_glass(out: &mut [u8], ow: u32, oh: u32, l: &CursorLens, accent: [u8; 3]) {
    let half = [
        (l.cbox[2] * 0.5 * l.squash).max(0.5),
        (l.cbox[3] * 0.5 * l.squash).max(0.5),
    ];
    let reach = half[0].hypot(half[1]) * 1.2;
    let acc = [accent[0] as f32, accent[1] as f32, accent[2] as f32];
    ...
    let cov = |px: f32, py: f32| -> f32 {
        ...
        l.mask
            .a
            .get((sy * l.mask.w + sx) as usize)
            .map_or(0.0, |&a| a as f32 / 255.0)
    };
    let b = [
        l.cbox[0] - span - DROP - 2.0,
        l.cbox[1] - span - 2.0,
        l.cbox[0] + span + DROP + 2.0,
        l.cbox[1] + span + DROP + 2.0,
    ];
    let src = snapshot(out, ow, oh, b);
    let c = [l.cbox[0], l.cbox[1]];
    for_box(ow, oh, b, |i, px, py| {
        let m = cov(px, py);
        let sh = cov(px, py - DROP) * 0.4
            + (cov(px - 1.5, py - DROP)
                + cov(px + 1.5, py - DROP)
                + cov(px, py - DROP + 1.5)
                + cov(px, py - DROP - 1.5))
        ...
        over(
            ...
            i,
            acc,
            INK * ink_cov((px - l.ink_at[0]).hypot(py - l.ink_at[1]), l.ink, reach) * m,
        );
    });
}
...
pub fn draw_lens(out: &mut [u8], ow: u32, oh: u32, l: &Lenses, accent: [u8; 3]) {
    if let Some(b) = &l.back {
        draw_back(out, ow, oh, b, accent);
    }
    if let Some(g) = &l.glass {
        draw_glass(out, ow, oh, g, accent);
    }
}
