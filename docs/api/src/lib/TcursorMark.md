...
**Reduced motion.** A local `useReducedMotion()` hook checks `matchMedia("(prefers-reduced-motion: reduce)")` directly (with a `typeof matchMedia === "function"` guard for any environment without it) and listens for live changes - independent of any Motion `MotionConfig` ancestor, since the editor tree has none (only the HUD wraps its tree in `<MotionConfig reducedMotion="user">`) and this mark needs to honor the OS preference wherever it mounts. When reduced, `seconds` and `pulsing` are forced to their off state regardless of `state`.
