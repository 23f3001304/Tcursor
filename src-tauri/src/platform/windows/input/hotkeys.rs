...
const TYPING_VKS: &[u32] = &[
    ...
    0x57, 0x58, 0x59, 0x5A, 0x20, 0x0D, 0x08, 0x09,
];
...
fn cur_mods() -> Mods {
    Mods {
        ctrl: key_down(VK_CONTROL.0.into()),
        alt: key_down(VK_MENU.0.into()),
        shift: key_down(VK_SHIFT.0.into()),
    }
}
...
mod tests {
    ...
    fn stop_returns_tuple_with_empty_vecs_when_no_input() {
        let tracker = Win32Hotkeys::start(vec![], Arc::new(PauseTotals::new()));
