...
The shared per-project-folder write lock (`doc_lock`) guarding every `edit.json` write, wherever it originates - `edit::commands`' three IPC entry points AND `seed_lock::load_or_seed`'s own internal seed/migrate/lift write (reached from ~8 other call sites across `ai`/`export`). See `docs/api/src-tauri/src/edit/lock.md` for the full ordering picture, including why it never inverts against `export::preview::session::WarmSlot`'s `gate` mutex.

## seed

Builds the first `EditDoc` from raw recording data (zoom regions and action log) so the editor opens in a state that matches today's exporter, then writes it as `edit.json`; on subsequent opens returns the existing file untouched. Holds the pure builders (`build_default`, `zooms_from_regions`, `layout_from_actions`, `actions_on_output_clock`) plus a `pub use` re-export of `seed_lock::load_or_seed` so callers keep using the `seed::` path.

## seed_lock

The load-or-create entry point (`load_or_seed`, self-locking against `edit::lock::doc_lock`) plus the two-phase fast-path/precompute-then-lock machinery that keeps that lock cheap (`load_or_seed_locked`, `derive_seed_inputs`, `needs_seed_write`) - see `docs/api/src-tauri/src/edit/seed_lock.md`.
