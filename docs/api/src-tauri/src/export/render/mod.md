...
- `texts: Vec<crate::edit::text::TextItem>` - the animated text items, on the OUTPUT clock like every `EditDoc` list and refreshed by `reload_edit`. Read by `fx_pass` at `pose.out_t`, together with `settings.ui.accent`, which is the only document-level input their look takes (the four styles are a fixed table in `export/fx/text/text_style.rs`).
...
- `clip_mix: ClipMixTrack` - the clip boundary cross-dissolve (`clipmix.md`), built from the RAW document's clip list by `EditState::load` and refreshed by `reload_edit`. It sits next to `map` because it is resolved against it: `walk_plan` calls `clip_mix.resolve(&self.map, fps)` once before the walk, and `step_camera` then reads one window lookup per frame into `FramePose.clip_mix`. Empty (no clips, or no `transition_in_ms`) for every document that has never split a take, and an empty track answers `None` at every instant, so the frame loop's blend branch is never entered.
- `grade: Option<GradeParams>` - the project's colour grade, already resolved from `settings.grade` by `EditState::load` and refreshed by `reload_edit`. `fx_pass` hands it to `fx_state::render`, which attaches it to the frame's `FxState`. *Why a resolved value and not the settings:* the settings are already here on `settings`, but turning them into parameters means a preset-table lookup, and the answer is the same for every frame of the export. *Why on the renderer at all rather than read off `self.settings` in `fx_pass`:* `params_of` returns `None` for the identity, and that `None` is what lets an ungraded export skip the FX pass entirely; deciding it once is what makes that free.
