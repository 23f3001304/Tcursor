...
pub fn hold_alpha(
    ...
    et: u32,
    fade_ms: u32,
    is_start: impl Fn(ActionKind) -> bool,
    is_end: impl Fn(ActionKind) -> bool,
) -> f32 {
        ...
        if is_start(a.kind) {
            ...
            if let Some(s) = start.take() {
                if et >= s {
                    ...
                    let down = if et >= a.t {
                        (1.0 - (et - a.t) as f32 / fade).max(0.0)
                    } else {
                        1.0
                    };
    ...
    if let Some(s) = start {
        if et >= s {
            alpha = alpha.max(((et - s) as f32 / fade).min(1.0));
        }
    }
    alpha
}
...
pub fn hold_spans(
    ...
    for a in actions {
        if is_start(a.kind) {
            ...
            if let Some(s) = start.take() {
                spans.push((s, a.t.max(s)));
            }
        }
    }
    if let Some(s) = start {
        spans.push((s, end_default.max(s)));
    }
    spans
}
