# src/editor/stage/cursor/cursorPreview.ts
...
- `src/editor/stage/canvas/previewCanvas.ts` - `drawPreview` passes the zoom-projected cursor position through to `drawCursorSprite`.
- `src/editor/hooks/stage/useCursorSprites.ts` - builds `CapturedLayer` from the `cursor_layer` DTO.
