...
fn add_cuts_is_one_op_that_lands_sorted_merged_and_with_ids() {
    ...
    apply(
        &mut d,
        EditOp::AddCuts {
            spans: vec![(5000, 6000), (1000, 2000), (1900, 2500)],
        },
    );
    let got: Vec<(String, u32, u32)> = d
        .cuts
        .iter()
        .map(|c| (c.id.clone(), c.start_ms, c.end_ms))
        .collect();
    assert_eq!(
        got,
        vec![("c0".into(), 1000, 2500), ("c1".into(), 5000, 6000)]
    );
}
...
fn add_cut_merges_into_an_existing_cut_and_keeps_the_earlier_id() {
    ...
    assert_eq!(
        (d.cuts[0].id.as_str(), d.cuts[0].start_ms, d.cuts[0].end_ms),
        ("c0", 1000, 3000)
    );
}
...
fn update_and_remove_cut_by_id_and_a_zero_length_result_is_dropped() {
    ...
    apply(&mut d, EditOp::RemoveCut { id: "c0".into() });
...
fn cuts_are_clamped_into_the_clip() {
    ...
    assert_eq!(
        got,
        vec![(9000, 10_000)],
        "a cut past the clip end is clamped; one entirely past it is dropped"
    );
}
...
fn speed_spans_never_overlap_and_the_factor_is_clamped() {
    ...
    let got: Vec<(u32, u32, f32)> = d
        .speed
        .iter()
        .map(|s| (s.start_ms, s.end_ms, s.factor))
        .collect();
    ...
    assert_eq!(
        got,
        vec![(1000, 3500, 2.0), (3500, 4000, 0.25)],
        "the updated span pushes its successor's start"
    );
