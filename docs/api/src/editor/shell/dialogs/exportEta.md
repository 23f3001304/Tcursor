# src/editor/shell/dialogs/exportEta.ts
...
- `ExportProgress` (`src/editor/shell/dialogs/ExportProgress.tsx`) - recomputed on every `pct` change while exporting, from an internally-tracked start timestamp.
