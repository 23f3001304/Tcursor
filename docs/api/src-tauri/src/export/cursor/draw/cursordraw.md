# src-tauri/src/export/cursor/draw/cursordraw.rs
...
- `src-tauri/src/export/cursor/pack/cursorset.rs` - stores one `CursorSprite` per `CursorType` in `CursorPrep.set`; calls `decode_sprite` at prep time and `apply_enhanced` per frame.
