...
/// The hold phase's OWN target for a camera centred at `c` with the cursor at `p` - the
/// `mx`/`my` clamp-window logic of `CameraSim::step`, mirrored here so a test can pin
/// "the ramp ends exactly where the follow phase would have put it" (zero discontinuity).
/// `half` is the frame half-extent on that axis, `s` the region's target scale.
...
fn static_cursor_zoom_matches_the_fixed_anchor_trajectory() {
    ...
    for t in (0..=2000).step_by(16) {
        let (a, b) = (sa.step(t, cur, &[fixed], &cfg), sb.step(t, cur, &[live], &cfg));
...
fn moving_cursor_zoom_lands_on_the_live_cursor_with_no_second_stage() {
    ...
    for t in (0..350).step_by(10) { prev = s.step(t, cur(t), &r, &cfg); samples.push(prev); }
    ...
    let hold = s.step(350, cur(350), &r, &cfg);
    ...
    // (3) Past zin_end, with the cursor parked, motion is only follow damping - which for a
    // cursor inside the dead band means no second stage at all.
    let mut last = hold;
    for t in (360..1000).step_by(10) {
        let c = s.step(t, cur(t), &r, &cfg);
    ...
    assert!((last.cx - hold.cx).abs() < 2.0 && (last.cy - hold.cy).abs() < 2.0,
...
fn retimed_cursor_zoom_ignores_the_stale_anchor() {
    ...
    for t in (0..=3500).step_by(10) {
        let (a, b) = (sa.step(t, cur(t), &stale, &cfg), sb.step(t, cur(t), &other, &cfg));
...
fn handoff_into_a_cursor_zoom_blends_toward_the_live_cursor() {
    ...
    // cursor - not the stored anchor. B's ramp is over by t=2200, so the blend has landed.
    let cfg = ZoomConfig::default();
    ...
    for t in (0..=2200).step_by(10) { c = s.step(t, cur, &r, &cfg); }
