...
Clears `Recorder::stopping` when `stop_take` leaves scope, however it leaves. *Why a guard and not a store at the end:* a panic anywhere in the teardown would otherwise leave the flag stuck on, and a stuck flag means every later `start_recording` returns `"already recording"` - the Record button dead for the rest of the session. Its `Drop` takes the `inner` lock before clearing, so "a start either sees `stopping`, or sees a fully torn-down recorder" is an ordering guarantee rather than an accident of timing.
...
Resolves the `Recorder` from the `AppHandle` and calls `stop_take`, handing it a closure that swaps the icon back. Two lines; everything else is below.
...
The whole body of the Stop, with no Tauri type in the signature, which is what lets `platform/mock/cycle_tests.rs` stop a take. Runs on a blocking-pool thread when the command drives it. Every handle it touches is thread-agnostic by construction: the Windows pointer port's `stop` and `CaptureControl::stop` post `WM_QUIT` to a *stored* thread id and then join, the hotkey and cursor-shape ports are an atomic flag plus a join, and `windows-capture`'s `VideoEncoder` is declared `Send` and does its muxing on its own transcode thread.

- `claimed: &dyn Fn()` - run once the take has actually been taken out of the `Recorder`, before any teardown. *Why a closure and not the `AppHandle`:* the only thing it does is `brand_icon::set_recording(app, false)`, and its POSITION is the whole point - a redundant Stop against an idle recorder must not touch the icon, so it cannot simply move up into the command. A `&dyn Fn()` is the same shape as the `Notify` the capture pipeline uses to keep Tauri out, one call site narrower.
...
2. Call `claimed()` - the command passes the `brand_icon::set_recording(app, false)` swap. Only reached once step 1 confirms a recording was actually taken, so a redundant Stop (already-idle) never touches the icon.
...
4. Build a `ProjectPaths` from `running.folder` (the captured cursor layer is several files, so `save_inputs` takes the paths rather than one more `&Path` argument) and call `save_inputs` with the three boxed input ports. *Why before the video stop:* these are cheap and must survive a video finalize failure.
5. `VideoSink::stop(running.video)` -> `VideoStopped` (GPU: end capture + `encoder.finish()`; ffmpeg: set halt + WM_QUIT to unblock the WGC thread + join), then `recorder_threads::save_session_files` with `stopped.frame_ts`, `events_ms`, the atomic audio start times (0 treated as absent) and `running.screen`. *Why the video error is carried in the struct instead of propagating here:* it used to propagate with `?`, which skipped `sync.json`, `project.tcursor` and the recents entry entirely - so an encoder/mux tail failure (disk full on a long take) left a folder with no manifest, which `open_project`'s `*.tcursor` filter cannot even select, and no `sync.json`, so `build_timeline` would have synthesised a timeline and muxed the mic ~800 ms early. The frames that were recorded are on disk either way, so they are written either way.
...
*Why `VideoSink::stop(running.video)` spelled out rather than `running.video.stop()`.* Both compile - `stop` takes `self: Box<Self>`, so a `Box<dyn VideoSink>` receiver resolves - but the trait-qualified form says at the call site that this consumes the pipeline and which trait's contract applies, in a function where three other `stop`s (the flag, the mic flag, and the input ports') are also in play.
