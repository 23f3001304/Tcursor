# src/editor/stage/cursorBusy.ts

Pack format v2's animated busy cursor, mirroring Rust `src-tauri/src/export/cursor/busy.rs` exactly. The export and the preview run the SAME math on the SAME output-clock timestamp, so a paused preview shows precisely the frame the export would write for that instant.
