// Split from commands.rs per repo convention (#[path] sibling test module).
// NOTE: `open_project` itself opens a real native dialog, so it is not exercised here (mirrors
// `pack_import_tests.rs`'s note about not driving real OS/app-data side effects from a unit
// test). Its pure core, `folder_from_manifest_path`, and the argv resolver `launch_project_from_argv`
// carry the actual logic and are fully covered against temp folders.
use super::*;

fn temp_dir(name: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join("tcursor_project_commands_test").join(name);
...
fn resolves_the_containing_folder_when_a_manifest_is_present() {
    ...
    ProjectManifest::new(1920, 1080).save(&manifest_path).unwrap();
...
fn falls_back_when_the_folder_has_no_manifest_at_all() {
    // Simulates pointing at a pre-existing recording folder made before this feature - there is
    // no project.tcursor on disk, yet resolution must still succeed (back-compat).
    let dir = temp_dir("no_manifest");
    let hypothetical_manifest_path = dir.join("project.tcursor"); // deliberately never written
...
fn launch_project_from_argv_resolves_a_tcursor_path() {
    ...
    let args = vec!["TCursor.exe".to_string(), manifest_path.to_string_lossy().into_owned()];
