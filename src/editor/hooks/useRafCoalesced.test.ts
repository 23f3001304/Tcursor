...
/** A controllable fake scheduler standing in for requestAnimationFrame/cancelAnimationFrame -
 *  `runFrame()` invokes whichever callback is currently queued (like a real frame firing). */
function fakeScheduler() {
  let nextId = 1;
  const queued = new Map<number, () => void>();
  const schedule = (cb: () => void) => { const id = nextId++; queued.set(id, cb); return id; };
  const cancelSchedule = (id: number) => { queued.delete(id); };
  const runFrame = () => { const cbs = [...queued.values()]; queued.clear(); cbs.forEach((cb) => cb()); };
...
describe("coalesce", () => {
  ...
  it("calls onCommit once per frame, with the LATEST value", () => {
    ...
    const c = coalesce(onCommit, schedule, cancelSchedule);
    c(1); c(2); c(3); // a burst within one frame - only one scheduled call
    runFrame();
  ...
  it("a call while a frame is already queued does not schedule a second one", () => {
    ...
    c(2);
    expect(pendingCount()).toBe(1); // still just one queued frame
  });
