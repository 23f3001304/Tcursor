# src-tauri/src/export/fx/fx_lens.rs
...
- `mask` - the silhouette the refraction is confined to (`fx_lensmask`). `Arc` because the settled per-kind masks are built once at prep time and shared by every frame.
...
The returned point is raw `WH_MOUSE_LL` desktop coordinates; `fx_lensbuild::desktop_to_out` puts it through the same three-step conversion every click hit takes, so the bar's far end lands exactly where that click's ripple would.
...
- `src-tauri/src/export/fx/fx_lensbuild.rs` - `lenses_at`, once per frame when the back is on.
