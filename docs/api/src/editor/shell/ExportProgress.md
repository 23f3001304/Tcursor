...
- `done: boolean` - whether the export finished successfully. *Why:* switches to the outcome view once `exporting` has gone false without an error. The separate success icon is gone: the wave is already flat at 100%, so the dot it left behind is what scales away as the checkmark draws itself in its place.
