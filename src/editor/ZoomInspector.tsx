...
export function ZoomInspector({ zoom, dur, onApply, onClose }: {
  ...
  return (
    <div className="e-panel e-insp">
      ...
      <div className="e-field2">
        <label className="e-field"><span className="e-fl">Zoom in (s)</span>
        ...
        <label className="e-field"><span className="e-fl">Zoom out (s)</span>
          ...
          {[
            ...
            <button key={c.name} className={`e-curve-card ${zoom.easing === c.key ? "on" : ""}`} onClick={() => upd({ easing: c.key })}>
              <svg viewBox="0 -30 100 160" className="e-curve-svg">
                <line x1="0" y1="0" x2="100" y2="0" stroke="var(--e-border, var(--line, #e4e4e7))" strokeDasharray="3 3" />
                <line x1="0" y1="100" x2="100" y2="100" stroke="var(--e-border, var(--line, #e4e4e7))" strokeDasharray="3 3" />
                <path d={c.path} fill="none" stroke={zoom.easing === c.key ? "var(--accent, #ef4444)" : "var(--fg, #000)"} strokeWidth="6" strokeLinecap="round" />
              </svg>
              <span>{c.name}</span>
            </button>
          ))}
