// Tests for settings::appearance, split out so appearance.rs stays under the size limit.
use super::*;
...
fn overlay_for_default_screen_is_today_bubble() {
    ...
    assert_eq!(o.width_px, 420);           // Square: width == height, byte-identical to today
...
fn overlay_for_default_camera_is_rounded_1920() {
    ...
    assert_eq!(o.width_px, 1920);         // Square default: width == height
...
fn overlay_for_wide_aspect_scales_width_only() {
    ...
    assert_eq!(o.size_px, 420);                                   // height unchanged
    assert_eq!(o.width_px, (420.0f32 * 16.0 / 9.0).round() as u32); // 747
}
#[test]
fn overlay_for_ring_scales_from_min_side() {
    let mut ma = AppearanceSettings::default().screen;
    ma.cam_ring = Some(CamRing { width: 0.05, color: [10, 20, 30] });
    let o = overlay_for(&ma, 3840, 2160, true);
    assert_eq!(o.ring_px, (0.05 * 420.0f32).round() as u32); // min(width_px, size_px) == 420 at Square
