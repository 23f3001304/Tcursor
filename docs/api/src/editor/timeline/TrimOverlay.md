...
**Threshold before any live movement (bug-sweep-2 Task 8, M8).** `move` ignores every pointermove until `pastDragThreshold(e.clientX - startXRef.current, 0)` (`../hooks/dragThreshold.ts`, default 3px) is true, at which point `movedRef.current` latches `true` for the rest of the drag - so a handle that never moved 3px never visibly updates `drag.ms` at all (no snap-back-on-release jitter for a sub-threshold jiggle).
