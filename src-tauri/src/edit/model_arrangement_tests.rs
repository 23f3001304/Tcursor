...
fn an_untouched_old_seg_re_serializes_byte_identically() {
    ...
    assert_eq!(
        out, OLD_SEG,
        "re-saving an untouched old segment must not change a byte"
    );
}
...
fn a_whole_old_doc_round_trips_without_growing_an_arrangement_key() {
    ...
    assert!(
        ...
        "unset arrangement must not be written: {}",
        json
    );
...
fn a_set_arrangement_round_trips_through_json() {
    ...
    assert!(
        json.contains("\"arrangement\""),
        "set arrangement must be written: {}",
        json
    );
...
fn a_hidden_panel_round_trips_as_an_explicit_null() {
    ...
    let json = serde_json::to_string(&s).unwrap();
    assert!(
        json.contains(r#""screen":null"#),
        "hidden panel must be an explicit null: {}",
        json
    );
