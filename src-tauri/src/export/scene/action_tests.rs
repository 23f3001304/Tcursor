...
fn stay_is_the_identity_at_every_scale() {
    for &s in &[1.0f32, 1.5, 2.2] {
        assert_eq!(
            apply_cam_zoom_action(panel(), CamZoomAction::Stay, s, 2.2),
            panel(),
            "scale {}",
            s
        );
...
fn hide_fades_alpha_out_as_the_zoom_deepens() {
    ...
    assert!(
        mid.alpha > 0.0 && mid.alpha < 1.0,
        "mid zoom -> partial, got {}",
        mid.alpha
    );
...
fn resolved_default_derives_from_the_legacy_shrink_fields() {
    ...
    let explicit = ZoomSettings {
        cam_zoom_default: Some(CamZoomAction::Hide),
        ..ZoomSettings::default()
    };
    assert_eq!(
        explicit.resolved_cam_action(),
        CamZoomAction::Hide,
        "explicit default beats legacy fields"
    );
}
...
fn cam_action_falls_back_to_the_global_default() {
    ...
    assert_eq!(
        cam_action_at(&[], &z, 500).0,
        CamZoomAction::Shrink { to: 0.62 },
        "no regions"
    );
    let rs = vec![region(0, 1000, 0, None)];
    assert_eq!(
        cam_action_at(&rs, &z, 500).0,
        CamZoomAction::Shrink { to: 0.62 },
        "region inherits"
    );
    assert_eq!(
        cam_action_at(&rs, &z, 5000).0,
        CamZoomAction::Shrink { to: 0.62 },
...
fn highest_layer_region_supplies_the_action_when_zooms_overlap() {
    ...
    assert_eq!(
        cam_action_at(&rs, &z, 1500).0,
        CamZoomAction::Hide,
        "layer 1 wins the overlap"
    );
    assert_eq!(
        cam_action_at(&rs, &z, 500).0,
        CamZoomAction::Stay,
        "only layer 0 is active here"
    );
}
...
fn cam_action_at_returns_the_winning_regions_own_scale_not_the_global_one() {
    ...
    assert_eq!(
        scale, 1.6,
        "must be the winning region's own target_scale, not the global 2.2"
    );
}
...
fn cam_action_at_falls_back_to_the_global_scale_when_no_region_is_active() {
    ...
    let rs = vec![region(0, 1000, 0, None)];
    assert_eq!(
        cam_action_at(&rs, &z, 5000).1,
        z.target_scale,
        "outside every region -> global"
    );
}
...
fn per_zoom_scale_reaches_full_effect_at_its_own_peak_not_the_global_one() {
    ...
    assert!(
        full.alpha.abs() < 1e-6,
        "expected full hide at the region's own peak scale (1.6): alpha={}",
        full.alpha
    );
}
...
fn settings_json_without_cam_zoom_default_loads_and_resolves_to_shrink() {
    let s: ZoomSettings =
        serde_json::from_str(r#"{"camera_shrink":true,"camera_shrink_min":0.62}"#).unwrap();
    assert_eq!(s.cam_zoom_default, None);
    assert_eq!(s.resolved_cam_action(), CamZoomAction::Shrink { to: 0.62 });
}
