...
// The five instants `src-tauri/src/export/cursor/busy_tests.rs` asserts the SAME numbers at. Any
// change here has to be made there in the same edit, or the export and the preview silently
// disagree about what the busy cursor is doing.
...
describe("busyPose parity with Rust busy_pose", () => {
  it("spins once per second at the default fps", () => {
    const expected = [0, 90, 180, 270, 0]; // 1000ms is a full turn, back to 0
...
describe("busyPose frames and edge cases", () => {
  it("lets explicit frames win over the synthesised animation", () => {
    ...
    for (const [t, want] of [[0, 0], [125, 1], [250, 2], [375, 3], [500, 0], [1000, 0]]) {
  ...
  it("stays in range for a negative time, the way rem_euclid does", () => {
    // A scrub can hand the preview a playhead before the clip's own origin.
