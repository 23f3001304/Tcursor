# src/editor/panels/layoutPresets.ts
...
- `src/editor/panels/LayoutsPanel.tsx` - the picker's default, the reset button, and the four preset write callbacks.
- `src/editor/panels/LayoutPresetList.tsx` - `BUILTIN_PRESET`, `MAX_PRESET_NAME` and `presetNameError` (the name field's `check`).
