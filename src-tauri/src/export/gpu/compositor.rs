...
#[path = "compositor_ring.rs"]
mod compositor_ring;
use compositor_ring::blit_ring;
...
pub trait Compositor: Send + Sync {
    ...
    /// Tell the compositor whether `bg` now CHANGES every frame (a video/GIF background). The
    /// default is a no-op: `CpuCompositor` copies `bg` per frame anyway, and only the GPU path
    /// caches it in a texture keyed by a sampled content hash - a key that a video frame moving
    /// only a small region can repeat, which would freeze the background on a stale upload.
