...
fn midpoint_lerp_with_linear_easing_is_the_exact_mean() {
    let moves = vec![
        kf(0, 0.0, 0.0, 0.0, "linear"),
        kf(1000, 1.0, 1.0, 1.0, "linear"),
    ];
...
fn midpoint_with_smooth_easing_diverges_from_the_linear_mean() {
    let moves = vec![
        kf(0, 0.0, 0.2, 0.1, "smooth"),
        kf(1000, 1.0, 0.2, 0.9, "smooth"),
    ];
    let track = CameraMoveTrack::from_doc(&moves);
    let f = ease(Easing::Smooth, 0.25);
    assert!(
        (f - 0.25).abs() > 1e-6,
        "sanity: smoothstep(0.25) must differ from linear 0.25"
    );
    let p = track.sample(250, None).unwrap();
    assert!(
        (p.x - f).abs() < 1e-6,
        "x should equal the eased fraction: got {} want {}",
        p.x,
        f
    );
...
fn mid_span_interpolation_is_unchanged_by_the_span_rewrite() {
    let moves = vec![
        kf(2000, 0.10, 0.80, 0.12, "smooth"),
        kf(4000, 0.70, 0.20, 0.34, "smooth"),
    ];
    let track = CameraMoveTrack::from_doc(&moves);
    let f = ease(Easing::Smooth, (3000.0 - 2000.0) / (4000.0 - 2000.0));
    let want = CamPose {
        x: 0.10 + (0.70 - 0.10) * f,
        y: 0.80 + (0.20 - 0.80) * f,
        size: 0.12 + (0.34 - 0.12) * f,
        round: None,
    };
    let live = CamPose {
        x: 0.95,
    ...
    for got in [
        track.sample(3000, None).unwrap(),
        track.sample(3000, Some(live)).unwrap(),
    ] {
    ...
    assert!(
        (want.x - 0.40).abs() < 1e-6,
        "pinned expected value drifted: {}",
        want.x
    );
}
...
fn shapes_morph_between_keyframes() {
    ...
    assert_eq!(
        shape_round("rounded", 9.0),
        Some(0.5),
        "roundness is clamped to a half side"
    );
}
...
fn a_layout_shape_inherits_from_the_live_pose() {
    ...
    let track =
        CameraMoveTrack::from_doc(&[shaped(2000, "layout", 0.0), shaped(4000, "circle", 0.0)]);
