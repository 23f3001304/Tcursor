# src/shared/wave/ui/QuietWaves.tsx
...
`.w-horizon` in `src/shared/wave.css` is the only place the `--e-wave` blue appears. That is deliberate: an empty state has no content and no interactive accent to compete with, which is the whole reason the motif can afford a second hue there and nowhere else.
