...
- `exporting: boolean`, `pct: number`, `done: boolean`, `error: string | null`, `exportPath: string` - passed straight through to `ExportProgress`; `exporting`/`done`/`error` are also used here to decide whether to show the settings form or the progress/outcome view (`showProgress = exporting || done || !!error`), and `done`/`error` gate `handleClose`'s reset (see Implementation).
...
- `Editor` (`src/editor/Editor.tsx`) - rendered once near `moveOffDialog`; `TopBar`'s `onOpenExport` sets the `open` state, and `onExport` wires to `exportProject(folder, settings)` after resetting `exportDone`/`exportError`/`exportPath` and setting `exporting`/`pct`. `onReset` likewise clears `exportDone`/`exportError`/`exportPath`.
