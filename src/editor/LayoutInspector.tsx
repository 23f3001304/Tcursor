...
import { NumberField, Picker } from "./Controls";
...
export function LayoutInspector({ seg, dur, onApply, onClose }: {
  ...
  return (
    <div className="e-panel e-insp">
      ...
      <p className="e-lede">Switches the frame layout from here. Drag the block to move the switch point.</p>
      ...
      <label className="e-field">
        <span className="e-fl">Transition (s)</span>
          ...
          {CURVES.map((c) => (
            <button key={c.name} className={`e-curve-card ${seg.easing === c.key ? "on" : ""}`} onClick={() => upd({ easing: c.key })}>
              <svg viewBox="0 -30 100 160" className="e-curve-svg">
                <line x1="0" y1="0" x2="100" y2="0" stroke="var(--e-border, #e4e4e7)" strokeDasharray="3 3" />
                <line x1="0" y1="100" x2="100" y2="100" stroke="var(--e-border, #e4e4e7)" strokeDasharray="3 3" />
                <path d={c.path} fill="none" stroke={seg.easing === c.key ? "var(--accent, #ef4444)" : "var(--fg, #000)"} strokeWidth="6" strokeLinecap="round" />
              </svg>
              <span>{c.name}</span>
            </button>
          ))}
