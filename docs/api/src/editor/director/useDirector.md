...
- **Render hygiene pass:** `requestCancel`/`aimAndPress`/`reveal`/`run` are now all `useCallback`'d (`aimAndPress` deps `[]`, `reveal` deps `[aimAndPress]`, `run` deps `[reveal]`) - every closure they capture bottoms out in a ref (`pointerRef`/`cancelRef`) or a `useState` setter (`setProgress`/`setPlanning`), both permanently stable, so `run`'s own identity now holds across every render of `Editor`, not just once per mount. This matters because `Editor.tsx`'s `onRun` wraps `director.run` directly, and `onRun` is a prop of `Transport` and `EditorPanels` (both `React.memo`'d as of the same pass) - before this, `run`'s fresh identity every render would have defeated their memo on every single `onRun`-carrying render regardless of anything else being stabilized.
