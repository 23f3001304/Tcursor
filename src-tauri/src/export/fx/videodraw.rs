use crate::export::fx::fx_state::VideoFx;
        ...
        for x in 0..ow {
            ...
            match v.mode {
                ...
                VideoFxMode::ScreenFocus => {
                    let inside = (x as f32) > mx
                        && (y as f32) > my
                        && (x as f32) < ow as f32 - mx
                        && (y as f32) < oh as f32 - my;
                    if !inside {
                        let k = 1.0 - a * 0.6;
                        for c in 0..3 {
                            out[i + c] = (out[i + c] as f32 * k).round() as u8;
                ...
                VideoFxMode::ColorPop => {
                    let l = 0.299 * out[i + 2] as f32
                        + 0.587 * out[i + 1] as f32
                        + 0.114 * out[i] as f32;
                    for c in 0..3 {
                        let p = (out[i + c] as f32 - l) * 1.6 + l;
                        out[i + c] =
                            (out[i + c] as f32 * (1.0 - a) + p.clamp(0.0, 255.0) * a).round() as u8;
                    }
                }
                VideoFxMode::NebulaWash => {
                    let tint = [120u8, 90, 255];
                    for c in 0..3 {
                        out[i + c] = (out[i + c] as f32 * (1.0 - a * 0.4)
                            + tint[2 - c] as f32 * a * 0.4)
                            .round() as u8;
                    }
                }
            }
