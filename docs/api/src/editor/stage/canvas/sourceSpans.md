# src/editor/stage/canvas/sourceSpans.ts
...
Linear interpolation - `a` at `t = 0`, `b` at `t = 1`. Used by `spanAt` below for the `fit` ratio cross-fade, and imported by `../timeline/model/layoutTrack.ts`'s `lerpRect` (panel rect/radius/alpha/ring-width cross-fade) so the two files share one implementation instead of two copies of the same one-liner.
...
- `src/editor/stage/fx/fxGeometry.ts` - `mapCanvas` composes it with the panel/zoom projection.
- `src/editor/timeline/model/layoutTrack.ts` - `layoutAt` carries the active `src` onto `PreviewLayout`.
