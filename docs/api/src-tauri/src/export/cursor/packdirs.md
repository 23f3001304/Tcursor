# src-tauri/src/export/cursor/packdirs.rs
...
Roots that may hold `rel` (an `assets/...` resource folder), most-specific first - the same shape, and the same reasoning, as `win::sys::proc::ffmpeg_candidates`.
...
- `src-tauri/src/export/cursor/pack_import.rs` - `unique_id`'s collision guard, and the destination for a new import
