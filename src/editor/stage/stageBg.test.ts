...
describe("coverRect", () => {
  it("covers the destination by cropping the long axis, never squashing", () => {
    expect(coverRect(1920, 1080, 1000, 1000)).toEqual([420, 0, 1080, 1080]); // wide source, square dest
    expect(coverRect(1080, 1920, 1000, 1000)).toEqual([0, 420, 1080, 1080]); // tall source, square dest
    expect(coverRect(16, 9, 32, 18)).toEqual([0, 0, 16, 9]); // same aspect: nothing cropped
  });
...
describe("bgAssetUrl", () => {
  it("is empty unless the kind actually draws an asset, so no stale file is ever fetched", () => {
    ...
    expect(bgAssetUrl("C:\\rec", "background/a.mp4", "video", src)).toBe("asset://C:\\rec\\background\\a.mp4");
    expect(bgAssetUrl("C:\\rec", "background/a.png", "image", src)).toBe("asset://C:\\rec\\background\\a.png");
    // The asset is KEPT while a wallpaper shows (so re-selecting needs no re-import) - but it
    // must not be loaded, or a hidden <video> would keep decoding a background nobody can see.
