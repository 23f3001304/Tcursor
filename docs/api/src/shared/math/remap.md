...
Whether two OUTPUT times are separated by a splice: their segments (`segOfOut`, the first whose output range has not ended) are `a < b` and some join between them has `clipEnd !== clipStart`. A cut and a clip join count; a speed-span edge, a contiguous clip join, a backwards step and a pair inside one segment do not, and neither does a trim edge (`segOfOut` is -1 outside the mapped range, so the trim's edges are the ends of the output rather than interior joins). Mirrors `TimeMap::crosses_boundary`, which replaced `gapContaining`/`crosses_cut` on both sides. The predicate for a caller holding two output times and no plan, though none exists today: the preview's transport asks nothing of the kind, a multi-clip document stepping by segment in `clipTick` and a single-clip one seeking through `playbackAction`. A caller walking `framePlan` uses `planBoundaries` instead. Kept as the pinned definition of the question itself - two output instants separated by a splice - whether or not it stays is the owner's call.
