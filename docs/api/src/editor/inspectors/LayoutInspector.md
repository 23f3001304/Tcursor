...
- **Header thumbnail** (T34 L4) - `InspectorHeader`'s `thumb` slot renders `LayoutThumb` (`../timeline/LayoutThumb.tsx`) at 40x23 (down from 48x28, which crowded the header's tighter single row) from the SAME `panels` (`resolvedPanelsFor(seg, presets)`) this file already computes for the visibility switches below, so the schematic and the switches can never disagree about what's actually shown.
