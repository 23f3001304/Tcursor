use super::*;
use crate::export::cursor::pack::packlist::list_packs;
use std::io::Write;
...
fn busy_resolves_to_arrow_bytes_and_hotspot_for_the_builtin_pack() {
    ...
    let builtin_busy = SPRITES
        .iter()
        .find(|&&(k, ..)| k == CursorType::Busy)
        .unwrap();
...
fn custom_png_overrides_one_kind_others_fall_back_to_builtin() {
    ...
    let builtin_hand = SPRITES
        .iter()
        .find(|&&(k, ..)| k == CursorType::Hand)
        .unwrap();
...
fn a_v2_pack_declares_its_busy_animation_and_keeps_its_own_busy_sprite() {
    use crate::export::cursor::draw::busy::BusyAnim;
...
fn every_shipped_pack_declares_a_usable_busy_animation() {
    for info in list_packs(None)
        .iter()
        .filter(|p| p.builtin && p.id != DEFAULT_PACK_ID)
    {
...
fn exactly_one_shipped_pack_is_a_lens_and_the_default_is_never_one() {
    ...
    let glass: Vec<&str> = shipped
        .iter()
        .filter(|p| is_glass(&p.id))
        .map(|p| p.id.as_str())
        .collect();
    assert_eq!(
        glass.len(),
        1,
        "exactly one shipped pack declares material: glass, got {glass:?}"
    );
    assert_eq!(
        shipped
            .iter()
            .find(|p| is_glass(&p.id))
            .map(|p| p.category.as_str()),
        Some("Glass and glow"),
        "and it is listed in the glass section"
    );
    for p in &shipped {
        assert_eq!(
            p.material,
            material(&p.id),
            "{} on the wire vs on disk",
            p.id
        );
    }
}
...
fn the_default_pack_still_exports_from_the_embedded_sprites_not_its_folder() {
    use crate::export::cursor::draw::cursordraw::{decode_sprite, draw_cursor};
    ...
    for ((kind, bytes, hot), &(sk, spng, shot)) in rows.iter().zip(SPRITES.iter()) {
        assert_eq!(*kind, sk);
        let (want_png, want_hot) = if sk == CursorType::Busy {
            (arrow.1, arrow.2)
        } else {
            (spng, shot)
        };
    ...
    let (_, embedded_png, embedded_hot) = SPRITES
        .iter()
        .find(|&&(k, ..)| k == CursorType::Arrow)
        .unwrap();
    ...
    assert_eq!(
        drawn, reference,
        "a drawn Default frame must be byte-identical to the embedded one"
    );
    assert!(
