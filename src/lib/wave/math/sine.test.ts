...
const spec = (o: Partial<SineSpec> = {}): SineSpec =>
  ({ w: 100, mid: 20, amp: 10, lambda: 40, phase: 0, ...o });
...
describe("sineY", () => {
  ...
  it("puts the crest a full amplitude ABOVE the midline a quarter period in", () => {
    ...
    expect(sineY(s, s.lambda / 4)).toBeCloseTo(s.mid - s.amp, 10); // SVG y grows downward
  ...
  it("scales the amplitude by the envelope, never the midline", () => {
    ...
    expect(sineY(s, 0)).toBeCloseTo(s.mid, 10);           // envelope 0 -> flat at the midline
...
describe("sinePath", () => {
  it("samples from 0 to exactly w, whatever the step divides into", () => {
    ...
    expect(d.split(" L ").length).toBe(Math.ceil(101 / 4) + 1); // n segments + the initial M
    expect(d.endsWith(sineY(spec({ w: 101 }), 101).toFixed(2))).toBe(true);
  });

  it("draws a flat line at zero amplitude", () => {
    const ys = sinePath(spec({ amp: 0, w: 20, step: 5 })).split(/[ML] /).slice(1).map((p) => p.trim().split(" ")[1]);
...
describe("crestBefore", () => {
  it("returns a real crest of the same wave, at or before the limit", () => {
    ...
    expect(sineY(s, x)).toBeCloseTo(s.mid - s.amp, 8); // a crest, not a trough
  });

  it("steps back by exactly one period as the limit crosses a crest", () => {
    const s = spec({ phase: 0, lambda: 40 }); // crests at 10, 50, 90, ...
