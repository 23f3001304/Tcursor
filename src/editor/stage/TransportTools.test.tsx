...
// What Cut and Speed 2x act on. The range is the deliberate answer; the click lookahead is the one
// that has to be right without a gesture, because it is what the buttons do on a first click.
const CLICKS: ClickSample[] = [{ t: 5000, x: 0, y: 0 }, { t: 20_000, x: 0, y: 0 }];
...
const click = (action: string) => act(() => { container.querySelector<HTMLElement>(`[data-action="${action}"]`)?.click(); });
const mount = (range: Range | null, timeMs: number, clicks: ClickSample[]) => act(() => {
  root.render(
    <TransportTools locked={false} trimmed={false} onTrimIn={() => {}} onTrimOut={() => {}} onResetTrim={() => {}}
      onAddZoom={() => {}} onAutoedit={() => {}} aiRunning={false} exporting={false}
      timeMs={timeMs} dur={60_000} clicks={clicks} range={range} setRange={(r) => ranges.push(r)}
      onApply={async (op) => { ops.push(op); return null; }} onDetectSilences={() => { silences += 1; }} />
  );
});

beforeEach(() => {
  (globalThis as unknown as { IS_REACT_ACT_ENVIRONMENT: boolean }).IS_REACT_ACT_ENVIRONMENT = true;
  ops = []; ranges = []; silences = 0;
...
describe("actionSpan", () => {
  ...
  it("and falls back to a 4 s block when the next click is further off than that", () => {
    ...
    expect(actionSpan(null, 9000, CLICKS, 60_000)).toEqual([9000, 13_000]); // 5000 is behind the playhead
  });
