...
// The label is portalled out of the wrapper (so no `overflow` ancestor can clip it), so it is
// looked for in the whole document rather than inside `container`.
const tip = () => document.querySelector(".e-tip");
const show = () => act(() => { root.render(<Tooltip label="Background"><button>bg</button></Tooltip>); });
const fire = (type: string) => act(() => { wrap().dispatchEvent(new MouseEvent(type, { bubbles: true })); });
...
describe("Tooltip", () => {
  it("stays out of the way until the pointer has rested on the control", () => {
    show();
    expect(tip()).toBeNull();
    fire("mouseover"); // React's onMouseEnter is delegated from mouseover
  ...
  it("shows at once on keyboard focus, since that control was reached deliberately", () => {
    show();
    act(() => { container.querySelector("button")!.dispatchEvent(new FocusEvent("focusin", { bubbles: true })); });
    expect(tip()?.textContent).toBe("Background");
  });

  // The bug this guards: rendered inline, the label was cut off by any ancestor with `overflow`
  // hidden/auto (a tile strip, `.e-panel`'s scroll box, the inspector column).
