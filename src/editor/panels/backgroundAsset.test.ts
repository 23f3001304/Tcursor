...
describe("assetSubtitle", () => {
  it("says what the asset is, with a duration only when there is one", () => {
    expect(assetSubtitle({ rel_path: "background/a.mp4", kind: "video", width: 1920, height: 1080, duration_ms: 4200 }))
      .toBe("Video 1920x1080, 4.2s");
    expect(assetSubtitle({ rel_path: "background/a.png", kind: "image", width: 2560, height: 1440, duration_ms: null }))
  ...
  it("drops a dimension it could not probe rather than printing 0x0", () => {
    expect(assetSubtitle({ rel_path: "background/a.mp4", kind: "video", width: 0, height: 0, duration_ms: 1000 }))
      .toBe("Video 1.0s");
  });
});
