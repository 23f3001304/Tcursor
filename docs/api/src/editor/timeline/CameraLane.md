...
**Transition curve.** Clicking a `.e-camseg` opens `.e-campop`, a popover of the shared `CURVE_GLYPHS` glyphs anchored at the segment midpoint; picking one applies `update_camera_move { id, easing }` to the later keyframe (easing eases *into* it) and keeps the popover open so curves can be auditioned against the live preview. It dismisses on Escape or an outside pointerdown (segments excepted, so clicking a neighbour re-anchors instead of closing).
