...
export function useSyncRefs({
  playing, timeMs, onTime, track, layout, clicks, effects, clickfx,
  captions, capStyle, accent, cursorKinds, cursor,
}: { /* one field per Stage prop that the composite loop needs live */ }): {
  playRef, timeRef, onTimeRef, trackRef, layoutRef, clicksRef, effectsRef, clickfxRef,
  captionsRef, capStyleRef, accentRef, kindsRef, cursorRef
}
```
