# src/editor/stage/cursorGlass.ts

The glass cursor material, as much of it as a 2D canvas can do. Split out of `cursorPreview.ts` for its size budget; the export's own version is Rust `export/fx/fx_lens.rs` plus `fx_lens.wgsl`, and every number here is that file's.
...
- `src/editor/stage/cursorPreview.ts` - `drawCursorSprite`, before the trail and the sprite.
