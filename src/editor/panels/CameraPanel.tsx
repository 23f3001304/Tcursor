...
import { camMoveAt, type CamPose } from "../stage/cameraMoves";
...
export function CameraPanel({
  ...
  // cam_size only applies where the track is empty/before the first keyframe.
  ...
  const setKfSize = (v: number) => {
    ...
    void commitCamKeyframe(doc.camera_moves, timeMs, d ? { x: d.x, y: d.y, size: v } : { size: v },
      { x: d?.x ?? kfPose?.x ?? 0.5, y: d?.y ?? kfPose?.y ?? 0.5, size: v }, applyOp);
  };
  ...
  const addKeyframeHere = () => {
    const p = camDraftRef.current ?? kfPose ?? { x: 0.5, y: 0.5, size: ma.cam_size };
  ...
  return (
    <div className="e-panel e-insp">
      ...
      <div className="e-field">
        <span className="e-fl">Move in preview</span>
        ...
        <>
          ...
          <div className="e-field" style={{ marginBottom: 0 }}>
            ...
            <Slider min={SLIDERS.cam_size.min} max={SLIDERS.cam_size.max} step={SLIDERS.cam_size.step}
              value={kfSize} onChange={setKfSize} />
          </div>
          ...
          {MODE_SLIDERS[mode].filter((k) => k.startsWith("cam_")).map((k) => {
            ...
            return (
              <div className="e-field" key={k} style={{ marginBottom: 0 }}>
                ...
                <Slider min={spec.min} max={spec.max} step={spec.step} value={ma[k]} onChange={(v) => set(k, v)} />
              </div>
            );
          })}
        </div>
      ...
      <button type="button" className="e-ghostbtn" style={{ marginTop: 16 }} onClick={() => onChange(DEFAULT_APPEARANCE)}>
        Reset Defaults
      </button>
    </div>
  );
