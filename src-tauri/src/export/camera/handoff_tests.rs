...
fn the_blend_still_lands_exactly_on_the_target() {
    for v in [-2.0f32, 0.0, 0.6, 3.0] {
        ...
        while let Some(c) = tr.blend(STEP, pose(900.0)) {
            last = c;
            n += 1;
        }
        let ran = n as f32 * STEP;
        assert!(
            (400.0..=400.0 + 2.0 * STEP).contains(&ran),
            "v={v}: the blend ran {n} frames ({ran:.1}ms), want ~400ms + the backdated step"
        );
        assert!(
            (last.cx - 900.0).abs() < 6.0,
            "v={v}: the last blended frame must be ~the target, got {}",
            last.cx
        );
        assert!(
            tr.blend(STEP, pose(900.0)).is_none(),
            "v={v}: a finished blend must stay finished"
        );
    }
}
...
fn a_moving_camera_is_not_stopped_dead_by_a_handoff() {
    ...
    assert!(
        after > before * 0.8,
        "the handoff still stalls the pan: {before:.2} -> {after:.2} px/frame"
    );
}
