...
/** The f64 value Rust's `1000.0f32 / 60.0` widens to. Spelled out rather than written as
 *  `1000 / 60` so this test and `tilt_tests.rs` feed the filter the SAME dt bits - otherwise the
 *  two drift by one 1 ms substep exactly where the angle is climbing fastest, and a pin comparison
 *  would be measuring float spelling rather than the filter. */
const STEP60 = 16.666666030883789;

/** The Rust `throw` harness: `speed` reference px/ms to the right until `moveMs`, then still. */
...
describe("the cursor's motion tilt", () => {
  it("matches the five pins Rust's tilt_tests.rs asserts", () => {
    // If these move, `the_five_pins_the_typescript_mirror_must_match` moves with them - or the
    // moving preview leans differently from the frame the backend hands back on pause.
    const r = throwRun(STEP60, 2.0, 200, 600, MAX_DEG);
    for (const [t, want] of [[100, 1.213484], [200, 3.291586], [300, 1.707726],
      [400, -0.444513], [600, 0.070651]]) {
  ...
  it("leans into a fast throw and never runs away past the cap", () => {
    ...
    for (const [tilt, speed] of [[1, 8], [0.35, 8], [0.35, 2]]) {
      const max = maxTiltDeg(tilt);
      for (const [, a] of throwRun(STEP60, speed, 600, 900, max)) expect(a).toBeLessThanOrEqual(max * 1.2);
      // It SETTLES on the cap; only the spring's swing on the way in goes past it.
  ...
  it("comes back upright through exactly one overshoot when the cursor stops", () => {
    ...
    const lows = r.filter(([, a], i) =>
      i > 0 && i < r.length - 1 && a < -0.2 && a <= r[i - 1][1] && a <= r[i + 1][1]);
  ...
  it("stays upright for ordinary pointing, and at tilt 0", () => {
    for (const speed of [0.05, 0.2, 0.39]) {
      for (const [, a] of throwRun(STEP60, speed, 2000, 2000, MAX_DEG)) expect(Math.abs(a)).toBeLessThan(1e-9);
    }
    for (const [, a] of throwRun(STEP60, 8, 600, 1200, maxTiltDeg(0))) expect(a).toBe(0);
    expect(maxTiltDeg(0)).toBe(0);
    expect(maxTiltDeg(2)).toBe(MAX_DEG); // clamped, not trusted
    expect(maxTiltDeg(NaN)).toBe(0); // a missing setting reads as off, never as a NaN cap
  });

  it("means the same thing at 30fps as at 60fps", () => {
    const a = throwRun(STEP60, 2.5, 400, 1200, MAX_DEG), b = throwRun(2 * STEP60, 2.5, 400, 1200, MAX_DEG);
  ...
  it("only records the position on a tick with no elapsed time", () => {
    ...
    expect(tiltStep(s, 200, 0, 0, MAX_DEG)).toBe(0); // a paused redraw
    expect(tiltStep(s, 200, 0, STEP60, MAX_DEG)).toBe(0); // differences against THAT position
    resetTilt(s);
...
describe("tiltFromCam", () => {
  it("reads the camera track's fractions as reference pixels", () => {
    // 2 reference px/ms to the right = 2/1920 of the screen content per ms.
    const s = newTilt(), direct = newTilt();
    for (let i = 0; i <= 24; i++) {
      const t = i * STEP60, x = 2 * Math.min(t, 200);
  ...
  it("leans a vertical throw at half the weight of a horizontal one", () => {
    const down = newTilt(), right = newTilt();
    for (let i = 0; i <= 36; i++) {
      const t = i * STEP60;
      tiltFromCam(down, 0.5, (2 * t * (16 / 9)) / REF_W, 16 / 9, STEP60, 1); // -> 2 ref px/ms down
