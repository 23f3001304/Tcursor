# src-tauri/src/events/cursortype.rs
...
- `src-tauri/src/events/cursortracker.rs` - the polling loop classifies the live cursor handle to a `CursorType` and appends changes
- `src-tauri/src/events/cursortype.rs` (`CursorTrack`) - stored as `(u32, CursorType)` pairs in the timeline
- `src-tauri/src/export/cursorset.rs` - selects the sprite to draw per frame based on `CursorType`
- `src-tauri/src/ai/commands.rs` / `src-tauri/src/ai/timeline.rs` - the AI director reads cursor shapes to annotate the activity timeline
...
- `src-tauri/src/session/recorder.rs` - built from the `CursorTypeTracker` result and saved to `cursor.json`
- `src-tauri/src/export/cursorset.rs` - loaded and queried via `type_at` per frame during export
- `src-tauri/src/ai/timeline.rs` - loaded for AI timeline annotation
