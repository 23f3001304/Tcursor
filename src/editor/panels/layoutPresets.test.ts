...
import {
  BUILTIN_PRESET, addPreset, layoutAtPlayhead, nextPresetId, presetNameError,
  removePreset, renamePreset, resetLayout,
} from "./layoutPresets";

const seg = (id: string, start_ms: number, end_ms: number, layout: string): LayoutSeg =>
  ({ id, start_ms, end_ms, layout, transition_ms: 300, easing: "smooth", transition_out_ms: 0, easing_out: "smooth" });

const look = (name: string): AppearanceSettings =>
  ({ ...DEFAULT_APPEARANCE, presenter: { ...DEFAULT_APPEARANCE.presenter, pad: name.length / 100 } });
...
describe("layoutAtPlayhead", () => {
  ...
  it("ignores a layout name this build does not know, and lets the last overlapper win", () => {
    ...
    expect(layoutAtPlayhead([seg("a", 0, 1000, "camera"), seg("b", 400, 1000, "presenter")], 500)).toBe("presenter");
  });
});
...
describe("the four list edits over a Settings-shaped value", () => {
  ...
  it("renames in place - same id, same position, same appearance", () => {
    ...
    expect(out.layout_presets.map((p) => [p.id, p.name])).toEqual([["lp1", "Loud"], ["lp2", "Quiet"]]);
    expect(out.layout_presets[0].appearance).toEqual(look("a"));
    expect(renamePreset(before, "lp1", "Quiet")).toBe(before); // clash
    expect(renamePreset(before, "nope", "Fine")).toBe(before); // unknown id
  });
