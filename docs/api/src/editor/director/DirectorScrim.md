...
**Stop pill.** A `motion.div.e-director-stop` (bottom-center, `Toast`-styled: `--e-card` bg, `--e-border`, `--e-shadow-pop`, 12.5px), inside its own `AnimatePresence` so it fades in/out (`y: 8 -> 0`, 0.16s) independently of the scrim. While `planning`, prefixes a small pulsing dot (`motion.span`, `opacity` `[0.35, 1, 0.35]` looped over 1.1s - an indeterminate indicator, since there's no step count yet) and shows `` `Asking ${model || "the model"}… first runs can take a while — Esc to stop` ``. Otherwise, text is `` `Directing · step ${progress.step}/${progress.total} — Esc to stop` `` once `progress` resolves, else `"Directing… — Esc to stop"`. Its own `pointerdown` calls `e.stopPropagation()` before `onCancel`, so clicking it doesn't ALSO fire the underlying scrim's handler.
