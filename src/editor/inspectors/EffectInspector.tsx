  ...
  return (
    <div className="e-field">
      ...
      <Slider
        ...
        ariaLabel={label}
        label={label}
        formatValue={(v) => (overridden ? `${Math.round(v * 100)}%` : `Default (${Math.round(v * 100)}%)`)}
      />
    </div>
  );
}
    ...
    <InspectorShell kind="fx">
      ...
      <Section title="Look">
        ...
        <OverrideField
          label="Dim"
          value={effect.dim}
          defaultValue={d.spotlight_dim}
          min={0.2}
          max={0.9}
          step={0.05}
          onToggle={(on) => upd({ dim: on ? d.spotlight_dim : -1 })}
          onChange={(dim) => upd({ dim })}
        />
        <OverrideField
          label="Radius"
          value={effect.radius}
          defaultValue={d.spotlight_radius}
          min={0.05}
          max={0.3}
          step={0.01}
          onToggle={(on) => upd({ radius: on ? d.spotlight_radius : -1 })}
          onChange={(radius) => upd({ radius })}
        />
        <OverrideField
          label="Feather"
          value={effect.feather}
          defaultValue={d.spotlight_feather}
          min={0.02}
          max={0.25}
          step={0.01}
          onToggle={(on) => upd({ feather: on ? d.spotlight_feather : -1 })}
          onChange={(feather) => upd({ feather })}
        />
        ...
        <Hint>
          Off keeps the webcam PiP lit while the spotlight dims everything else. Applies to all spotlights.
        </Hint>
      </Section>
