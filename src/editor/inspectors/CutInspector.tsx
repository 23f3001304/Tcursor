...
export function CutInspector({ cut, dur, onApply, onClose }: {
  ...
  return (
    <InspectorShell kind="cut">
      <PanelHeader title="Cut" lede={spanLede(cut.start_ms, cut.end_ms)} closeTitle="Deselect" onClose={onClose} />
      ...
      <RemoveButton label="Remove cut" onClick={() => { void onApply({ op: "remove_cut", id: cut.id }); onClose(); }} />
    </InspectorShell>
  );
}
