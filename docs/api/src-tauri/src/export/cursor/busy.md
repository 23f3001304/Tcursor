# src-tauri/src/export/cursor/busy.rs
...
`src/editor/stage/cursorBusy.ts` is the TS mirror. Both are tested at the same five instants (0, 250, 500, 750, 1000 ms) against the same numbers; a change to one has to be made in the other in the same edit.
...
- `src-tauri/src/export/cursor/cursorset.rs` - `posed`, per exported frame
- `src-tauri/src/export/cursor/pack.rs` - `busy_spec` reads it out of `pack.json`
- `src/editor/stage/cursorBusy.ts` - the TS mirror, for the canvas preview and the panel's hover grid
