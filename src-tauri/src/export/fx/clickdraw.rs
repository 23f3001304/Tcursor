use crate::export::fx::clickfx::{ease_out, fade_alpha, ripple_radius, smoothstep};
...
/// Visit every pixel within `rad` of the segment `a`..`b` with its distance to that segment -
/// `fx_clicks.wgsl::ck_seg`, walked over the bounding disc of the capsule instead of the frame.
fn for_seg(ow: u32, oh: u32, a: (f32, f32), b: (f32, f32), rad: f32, mut f: impl FnMut(usize, f32)) {
    let (bx, by) = (b.0 - a.0, b.1 - a.1);
    let len2 = (bx * bx + by * by).max(1e-4);
    let half = len2.sqrt() * 0.5;
    for_disc(ow, oh, (a.0 + b.0) * 0.5, (a.1 + b.1) * 0.5, half + rad + 1.0, |i, _| {
...
/// `a` is NOT capped at 1: the shader clamps only the final colour (`min(color, 1.0)`), so a
/// boosted additive gain like Neon's `a * 1.4` must saturate the channel, not the weight.
...
/// Soft additive bloom: gaussian falloff to radius `r` (`ck_gauss`, out to 2r like the shader's
/// effective reach).
...
/// The impact flash every style shares: a white bloom 1.5% of the height wide, gone by p = 0.14
/// (about 80 ms of the 600 ms life).
fn flash(out: &mut [u8], ow: u32, oh: u32, cx: f32, cy: f32, p: f32, inten: f32) {
    let g = (1.0 - smoothstep(0.0, 0.14, p)) * inten;
    if g > 0.0 { glow(out, ow, oh, cx, cy, oh as f32 * 0.015, WHITE, g); }
}

/// `fx_clicks.wgsl`'s `ck_hash`. The GPU's `sin` is an approximation and this hash is chaotic in
/// its input, so individual sparks land on different pixels than the shader's - what this mirrors
/// is the LOOK: pseudo-random angles and per-spark speeds, not an evenly-spaced ring.
fn hash1(x: f32) -> f32 { ((x * 127.1).sin() * 43758.5453).fract() }

/// 14 additive sparks flung out from (cx,cy), arcing down with progress, each drawn as a streak
/// back along its own velocity over the last 0.06 of progress.
fn particles(out: &mut [u8], ow: u32, oh: u32, cx: f32, cy: f32, prog: f32, c: [u8; 3], a: f32) {
    ...
    for k in 0..14u32 {
        ...
        for_seg(ow, oh, (pos.0 - vel.0, pos.1 - vel.1), pos, r.max(1.0), |i, d| {
            let cov = (r - d).clamp(0.0, 1.0); // fx_clicks.wgsl: clamp(oh*0.004 - seg_dist, 0, 1)
            if cov > 0.0 { add_blend(out, i, c, g * cov); }
        });
    }
}

/// Bright additive ring (neon / shockwave / the rims).
...
/// Pulse's body: solid to `radius * 0.2`, smoothstepping away to nothing at `radius`.
...
/// A small hard additive disc with a 1px AA edge (`clamp(radius - d, 0, 1)`), Pulse's white core.
...
fn u8c(v: [f32; 3]) -> [u8; 3] { [(v[0] * 255.0) as u8, (v[1] * 255.0) as u8, (v[2] * 255.0) as u8] }

/// Draw the click effects for `state`. Mirrors `fx_clicks.wgsl` in geometry and gain for every
/// style; the ONE thing it cannot mirror is Shockwave's refraction + chromatic dispersion, which
/// resample the background (this path composites in place, with no source to re-sample), so
/// Shockwave here is its ring and rim alone - the same trade the file has always made.
    ...
    for hit in &state.hits {
        ...
        match state.style {
            ClickFxStyle::Ripple => {
                ...
                if p > 0.15 { ring(out, ow, oh, x, y, ripple_radius(p - 0.15, h * 0.06), h * 0.0045, c, a * 0.7); }
                if p > 0.30 { ring(out, ow, oh, x, y, ripple_radius(p - 0.30, h * 0.06), h * 0.003, c, a * 0.45); }
            }
            ClickFxStyle::Pulse => {
                let r = h * (0.01 + 0.025 * ease_out(p));
                soft_disc(out, ow, oh, x, y, r, c, a);
                ring_add(out, ow, oh, x, y, r, h * 0.004, c, a * 0.6);
                disc_add(out, ow, oh, x, y, h * 0.006, WHITE, a * (1.0 - smoothstep(0.0, 0.3, p)));
            }
            ClickFxStyle::Glow => {
                let r = h * 0.05 * (0.6 + 0.8 * p) * (1.0 + 0.06 * (p * std::f32::consts::TAU * 2.0).sin());
                glow(out, ow, oh, x, y, r, c, a);
                glow(out, ow, oh, x, y, h * 0.008, WHITE, a * 0.5);
            }
            // Two tubes, the second hue-rotated and launched 72 ms later; each is the tint at
            // 1.4x/1.0x PLUS a white core, which is what blows the line out to neon rather than
            // leaving it a plain bright ring. The gains are uncapped on purpose (see `add_blend`).
            ClickFxStyle::Neon => {
                ...
                if p > 0.12 {
                    ...
                    ring_add(out, ow, oh, x, y, r2, h * 0.008, u8c(hue_shift(c, NEON_HUE_SHIFT)), a);
