...
**Why one shared aim is the whole point.** The jank probe's largest measured defect (H1b, `-130` screen px/frame^2 on the 12s scene, `jank_phase_tests::h1_ramp_in_to_hold_on_an_anchored_region`) was that the ramp-in and the hold aimed at *different things*: the ramp eased toward `r.anchor`, while the hold only ever aimed at a dead band around the LIVE camera centre. On the ramp's last step the target teleported from the anchor to the dead-band edge and the centre reversed at `k * (tx - cx)` - one frame of `-102` px after one frame of `+27`. Making both phases call the same pure function of `(region, cursor)` removes the discontinuity by construction: the aim does not depend on `self.cx`, so it cannot change when the phase does.
