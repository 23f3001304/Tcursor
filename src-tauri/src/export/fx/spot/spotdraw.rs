...
pub fn draw_spot(out: &mut [u8], ow: u32, oh: u32, s: &Spot, intensity: f32) {
    ...
    let breathe = if s.mode == SpotlightMode::Breathing {
        1.0 + 0.12 * (s.t * 3.1416).sin()
    } else {
        1.0
    };
    ...
    for y in 0..oh {
        for x in 0..ow {
            ...
            .clamp(0.0, 1.0);
            let factor = if s.mode == SpotlightMode::Nebula {
                0.85
            } else if s.mode == SpotlightMode::Blur {
                1.15
            } else {
                1.0
            };
            ...
            if s.mode == SpotlightMode::Halo {
                ...
                if band > 0.0 {
                    add_tint(out, i, s.tint, band * inten);
                }
            }
            if keep_cam {
                ...
                if cov > 0.0 {
                    for c in 0..3 {
                        out[i + c] = (out[i + c] as f32 + (pre[c] as f32 - out[i + c] as f32) * cov)
                            .round() as u8;
                    }
                }
            }
...
mod tests {
    ...
    fn vignette_dims_corner_not_center() {
        ...
        assert!(
            ...
            "vignette darkens corners vs center"
        );
    }
    #[test]
    ...
    fn halo_ring_scales_with_intensity_like_the_shader() {
        ...
        assert!(
            half < full,
            "intensity 0.5 must dim the halo ring ({half} vs {full})"
        );
    ...
    fn dim_camera_false_keeps_camera_rect_lit() {
        ...
        assert_eq!(
            out[center_i], 200,
            "cam-rect center stays at full brightness when dim_camera is false"
        );
    }
    #[test]
    fn dim_camera_true_dims_the_camera_rect_too() {
        ...
        assert!(
            out[0] < 200,
            "dim_camera:true -> today's behavior, camera rect dims like everything else"
        );
    }
}
