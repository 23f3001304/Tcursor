...
fn spin_turns_once_per_second_at_the_default_fps() {
    let expected = [0.0, 90.0, 180.0, 270.0, 0.0];
...
fn spin_fps_sets_the_cycle_length() {
    close(
        busy_pose(&spec(BusyAnim::Spin, 12.0), 500).angle_deg,
        90.0,
        "half-speed spin @500ms",
    );
    close(
        busy_pose(&spec(BusyAnim::Spin, 48.0), 250).angle_deg,
        180.0,
        "double-speed spin @250ms",
    );
}
...
fn flip_holds_then_turns_and_never_turns_back() {
    ...
    close(
        busy_pose(&spec(BusyAnim::Flip, 24.0), 2000).angle_deg,
        0.0,
        "two half turns is a full circle",
    );
}
...
fn the_still_pose_takes_the_untransformed_fast_path() {
    ...
    assert!(
        busy_pose(&spec(BusyAnim::Spin, 24.0), 0).is_identity(),
        "angle 0 is still the fast path"
    );
...
fn anim_names_round_trip_through_the_pack_json_wire_form() {
    for (v, wire) in [
        (BusyAnim::Spin, "spin"),
        (BusyAnim::Flip, "flip"),
        (BusyAnim::Pulse, "pulse"),
    ] {
        assert_eq!(serde_json::to_string(&v).unwrap(), format!("\"{wire}\""));
        assert_eq!(
            serde_json::from_str::<BusyAnim>(&format!("\"{wire}\"")).unwrap(),
            v
        );
    }
