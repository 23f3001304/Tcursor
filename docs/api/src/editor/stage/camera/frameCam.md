# src/editor/stage/camera/frameCam.ts
...
- **A keyframe or drag is active** - `overrideCamPanel` replaces the rect and sets the radius from the pose's own shape (a keyframed circle/rounded/rect, morphing between keyframes) or, for a shapeless pose like the drag draft, scales it with the ring width by the height ratio, so a circle stays round. `live` (`liveCamPose(base.cam, ow, oh)` - the layout-resolved centre, height and corner fraction this frame) is what `camMoveAt` eases out of entering the keyframe span and back into leaving it, and the shape a `"layout"` keyframe inherits. Since Task 27 a keyframe is active only INSIDE that span; outside it `camMoveAt` is `null` and this branch is skipped, so the smart zoom action below applies as if there were no keyframes. A live Move-mode drag still wins unconditionally, span or not.
