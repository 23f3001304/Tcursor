# src-tauri/src/platform/windows/input/hotkeys.rs

Tracks hotkey chords and typing activity by polling the Windows async key state at ~60 Hz, instead of installing a `WH_KEYBOARD_LL` hook that security software silently swallows on some machines. Runs on a dedicated background thread owned by `Win32Hotkeys`; all cross-thread state is `Arc`-wrapped.

This is the `HotkeyPort` implementation for Windows (the trait impl itself is in `mod.rs`). `actions::keyboard::KeyboardTracker` is an alias for `Win32Hotkeys` so the recorder's call sites keep their spelling until the composition root is wired. What stays portable is in `actions/`: `Arm`, `Mods` and `ActionEvent`. What is here is the Win32 half - the virtual-key numbers and the `GetAsyncKeyState` reads - which a macOS or Linux adapter replaces wholesale.
...
- `src-tauri/src/platform/windows/input/mod.rs` - the `HotkeyPort` impl and `Win32Input::hotkeys`
- `src-tauri/src/session/record/recorder.rs` - stores it in the `Recorder` struct under the alias `KeyboardTracker` and creates it via `KeyboardTracker::start(arming_from_settings(&snap.hotkeys), ..)`.
...
5. Loop on `!s.load(SeqCst)`:
   - Read `cur_mods()` (`VK_CONTROL`/`VK_MENU`/`VK_SHIFT` via `GetAsyncKeyState`; the three codes are imported from `windows::Win32::UI::Input::KeyboardAndMouse`, never re-declared as hex - `TYPING_VKS` stays a raw table because naming 40 codes would be longer and no clearer).
