...
import {
  ...
  DEFAULT_APPEARANCE,
  type ModeKey,
} from "../preferences/appearanceFields";
...
function Seg<T extends string>({ items, on, pick }: { items: [T, string][]; on: T; pick: (v: T) => void }) {
  return (
    <div className="seg">
      {items.map(([v, l]) => (
        <button key={v} type="button" className={`seg-btn ${on === v ? "on" : ""}`} onClick={() => pick(v)}>
          {l}
        </button>
      ))}
    </div>
...
export function SettingsAppearance({
  ...
  return (
    <section className="sec">
      ...
      {MODE_HAS_SHAPE[mode] && (
        <>
          <Field label="Webcam shape">
            <Seg items={SHAPES} on={ma.cam_shape} pick={(v) => set("cam_shape", v)} />
          </Field>
      ...
      {MODE_HAS_CORNER[mode] && (
        <Field label="Webcam corner">
          <Seg items={CORNERS} on={ma.cam_corner} pick={(v) => set("cam_corner", v)} />
        </Field>
      )}
      <button
        type="button"
