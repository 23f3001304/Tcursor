...
describe("alphaBounds", () => {
  it("finds the drawn content inside a transparently padded sprite", () => {
    expect(alphaBounds(buffer(16, 16, { x: 3, y: 5, w: 4, h: 6 }), 16, 16)).toEqual({
      x: 3,
      y: 5,
      w: 4,
      h: 6,
...
describe("fitBox", () => {
  it("scales by the content box, so differently padded packs match", () => {
    ...
    expect(small.width / (128 / 40)).toBeCloseTo(20, 10);
  ...
  it("fits the limiting axis and never distorts", () => {
    ...
    expect(fit.height / fit.width).toBeCloseTo(1, 10);
    expect(fit.width).toBeCloseTo(100 * (24 / 100), 10);
  });
