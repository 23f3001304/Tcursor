...
pub struct ModeAppearance {
    ...
    pub cam_margin_x: f32,
    pub cam_margin_y: f32,
    pub cam_aspect: CamAspect,
    #[serde(default)]
    pub cam_ring: Option<CamRing>,
}
impl Default for ModeAppearance {
    fn default() -> Self {
        Self {
            ...
            screen_radius: 0.016,
            cam_size: 0.1944,
            cam_shape: CamShape::Circle,
            cam_radius: 0.04,
            cam_corner: CamCorner::BottomLeft,
            cam_margin_x: 0.0208,
            cam_margin_y: 0.037,
...
pub fn overlay_for(ma: &ModeAppearance, ow: u32, oh: u32, enabled: bool) -> OverlayLayout {
    let shape = match ma.cam_shape {
        ...
        CamShape::Rounded => OverlayShape::Rounded {
            frac: ma.cam_radius,
        },
    ...
    let (ring_px, ring_color) = match ma.cam_ring {
        Some(r) => (
            (r.width * width_px.min(size_px) as f32).round() as u32,
            r.color,
        ),
        None => (0, [0, 0, 0]),
    };
