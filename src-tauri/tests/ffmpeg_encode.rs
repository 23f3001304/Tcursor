...
fn encodes_synthetic_frames_to_nonempty_mp4() {
    ...
    // 320x240 (not 16x16) so GPU hardware encoders accept the dimensions.
    ...
    for i in 0..n {
        ...
        for px in bgra.chunks_mut(4) { px[2] = (i * 20) as u8; px[3] = 255; }
        sink.push(&Frame { width: w, height: h, bgra, ts: Timestamp((i * 33) as u64) }).unwrap();
    }
    sink.finish().unwrap();
