# src/editor/panels/camera/CameraPanel.tsx
...
```tsx
export function CameraPanel({ settings, onClose, doc, timeMs, applyOp, moveMode, onMoveModeChange, camDraftRef, hasWebcam }: {
...
- `hasWebcam: boolean` - `hasWebcamSignal(layout)` (`model/editorData.md`), threaded from `Editor` through `ClassicShell` and `EditorPanels` exactly as the timeline's `CameraLane` gets it. When false the move field is not rendered at all: the panel shows one `.e-hintline` ("This recording has no webcam track, so there is nothing to move. Record with the camera on to keyframe it.") in its place (panel pass, 2026-09-15). A Move switch that armed a drag handle over an empty corner was a control that did nothing, and the panel is honest about it the same way the lane's empty hint is gated on the same flag.
