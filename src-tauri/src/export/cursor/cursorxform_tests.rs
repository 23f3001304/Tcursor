...
/// Hotspot dead centre of a 3x3 sprite, so a rotation about it maps texel to texel.
const ANCHOR: (f32, f32) = (5.5, 5.5);

/// A 3x3 sprite whose RED channel identifies each texel: (x, y) holds `10 * (y * 3 + x) + 10`.
fn ids_3x3() -> CursorSprite {
    let mut bgra = Vec::new();
    for y in 0..3u8 {
        for x in 0..3u8 {
            bgra.extend_from_slice(&[0, 0, 10 * (y * 3 + x) + 10, 255]); // BGRA
        }
    }
    CursorSprite { bgra, w: 3, h: 3, hot: (0.5, 0.5), canvas_h: 3 }
}

fn frame() -> Vec<u8> { vec![0u8; (OW * OW * 4) as usize] }

/// The red channel at output pixel `(x, y)` - the sprite texel id that landed there.
fn id(out: &[u8], x: u32, y: u32) -> u8 { out[((y * OW + x) * 4 + 2) as usize] }
...
fn a_quarter_turn_clockwise_maps_every_texel_exactly() {
    ...
    for i in 0..3u32 {
        for j in 0..3u32 {
            ...
            assert_eq!(id(&out, 4 + i, 4 + j), want, "output ({}, {})", 4 + i, 4 + j);
        }
    }
    // Nothing outside the 3x3 footprint.
...
fn the_hotspot_stays_on_the_anchor_through_a_pulse() {
    // `pulse` scales about the hotspot, so the anchor texel must not move: the centre texel (id
...
fn a_rotated_edge_blends_without_a_halo_from_transparent_padding() {
    // One opaque WHITE texel in a 3x3 of fully transparent BLACK. Sampling straight-alpha colour
    // would average that black into every partially covered edge pixel - the classic dark halo.
    // Premultiplied sampling must not: composited over a black frame, a pure white source leaves
    // each channel EQUAL to the coverage it was blended at, so `rgb == alpha` is the halo check.
    ...
    assert!(touched > 1, "the rotated texel should straddle several pixels, got {touched}");
}
