...
wgpu device/pipeline initialization, GPU availability probe, and texture upload helpers. A process-global device + queue (`shared_device`) is created once and reused by every `Gpu`/`GpuFx` and across renderer rebuilds (aspect changes, new exports, the first preview), so only the dims-dependent pipeline/output-texture/readback-buffer allocation in `Gpu::new` repeats each time. Key items: `Gpu` struct (device, queue, pipeline, output texture, readback buffer), `Gpu::new(out_w, out_h) -> Option<Gpu>`, `shared_device() -> Option<(Arc<Device>, Arc<Queue>)>`, `Gpu::create_tex`/`create_tex_fmt`/`update_tex`/`update_tex_bpp`/`upload_tex`, `gpu_available() -> bool`, `FORMAT` constant (`Bgra8Unorm`), `align_up`.
