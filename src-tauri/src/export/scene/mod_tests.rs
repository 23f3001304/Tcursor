...
// Task 9 Part C: override_camera must scale radius (and ring) by the height ratio so a
// circle panel (radius == min(w,h)/2 at its STATIC size) stays a true circle after a
// keyframe grows/shrinks it, instead of leaving radius at the pre-override value.
#[test]
fn override_camera_keeps_circle_round_after_resize() {
    ...
    let out = override_camera(panel, p, 1000.0, 1000.0);
...
fn override_camera_matches_rect_from_center_position() {
    ...
    let (ow, oh) = (1920.0, 1080.0);
    let plain = rect_from_center(p, ow, oh);
    let out = override_camera(panel, p, ow, oh);
...
// the actual wiring (None for `&[]`, regardless of static_pose - the emptiness check runs
// first) is exercised by export::camera::moves_tests::empty_track_samples_to_none.
#[test]
