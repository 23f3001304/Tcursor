...
export function CaptionsPanel({
  ...
  return (
    <div className="e-panel e-insp">
      <PanelHeader title="Captions" lede="Generate subtitles and configure caption overlay rules." onClose={onClose} />
      ...
      {generating && (
        <div style={{ padding: "16px 0", textAlign: "center" }}>
          ...
          <div style={{ height: 4, background: "var(--e-soft)", borderRadius: 2, overflow: "hidden", margin: "12px 0 6px" }}>
            <motion.div initial={false} animate={{ width: `${progress}%` }}
              transition={{ type: "tween", duration: 0.15, ease: "easeOut" }}
              style={{ height: "100%", background: "var(--e-ai)" }} />
          </div>
      ...
      {!generating && subtitles.length > 0 && (
        <div className="e-field">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span className="e-sechead" style={{ margin: 0 }}>Editable Captions</span>
            <button type="button" className="e-reset" onClick={() => setSubtitles([])}>Clear</button>
          </div>
