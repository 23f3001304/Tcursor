...
import type { LayoutSeg, PanelPose } from "../../../lib/edit";
import type { LayoutPresets, PanelRectDto } from "../../../lib/ipc";
import { draftPanels, poseOfRect, setArrangementOp, withDraftSeg, type Panels, type Rect } from "./arrangeMath";

// The presentation/op-builder half of arrangeMath - the live draft that stands in for a resolved
// segment mid-drag, and the exact `set_arrangement` wire form a gesture commits. The gesture math
// itself (drag -> pose, resize anchoring, snapping) is `arrangeMath.test.ts`.
const panel = (rect: Rect, over: Partial<PanelRectDto> = {}): PanelRectDto =>
  ({ rect, radius: 0.02, alpha: 1, ring_px: 0.004, ring_color: [1, 2, 3], ...over });
...
const seg = (over: Partial<LayoutSeg> = {}): LayoutSeg => ({
  id: "s1", start_ms: 0, end_ms: 1000, layout: "camera", transition_ms: 0, easing: "smooth",
...
describe("draftPanels", () => {
  ...
  it("replaces only the dragged panel's rect and forces it visible", () => {
    ...
    expect(out.cam).toEqual(panel(CAM)); // untouched
  });
  ...
  it("scales the CAM radius and ring by the height ratio, so a circle stays round mid-drag", () => {
    const out = draftPanels(base(), "cam", [0.5, 0.5, 0.4, 0.4]); // 0.2 -> 0.4 high
...
describe("withDraftSeg", () => {
  ...
  it("appends an entry for a segment that has none yet, leaving the others alone", () => {
    ...
    expect(presets.segs).toHaveLength(1); // input untouched
  });
...
describe("setArrangementOp", () => {
  it("touches only the dragged panel once the segment already has an arrangement", () => {
    ...
    expect("cam" in op).toBe(false); // absent = "leave that panel alone"
  });
  ...
  it("fills the untouched panel in on a FIRST write (the backend bases that off 'both hidden')", () => {
    ...
    expect(op.screen).toEqual(poseOfRect(RECT)); // the screen keeps exactly where it resolves now
  });
