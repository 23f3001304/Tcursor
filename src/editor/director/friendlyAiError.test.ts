...
describe("friendlyAiError", () => {
  it("maps the no-models-installed failure (build_plan's exact wording)", () => {
    expect(friendlyAiError("No Ollama models are installed. Pull one first, e.g.: ollama pull llama3.2"))
  ...
  it("maps the unreachable-Ollama failure (ollama::chat's exact wording)", () => {
    expect(friendlyAiError("Could not reach Ollama at localhost:11434. Is it running? Try: ollama serve"))
  ...
  it("passes any other error through unchanged, with no hint", () => {
    expect(friendlyAiError("parse error: unexpected token")).toEqual({ title: "parse error: unexpected token", hint: null });
  });
});
