...
**Drop (insert from the palette).** `.e-tlbody` also carries `onDragOver` (`preventDefault` + `dropEffect = "copy"` - without the `preventDefault` the browser refuses the drop) and `onDrop`, which reads the `text/plain` payload the `EffectPills.tsx` cards set on `dragstart` and switches on it: `"zoom"` -> `add_zoom`, `"spotlight"` -> `add_effect`, `"layout"` -> `add_layout_seg`, `"cammove"` -> `add_camera_move`, `"blur"` / `"pixelate"` / `"highlight"` -> `add_effect` with that kind (a 3s region, matching `useTimelineActions::addMask`), and anything beginning `"text:"` -> `add_text` with the rest of the payload as the kind, each at the drop x mapped into `[0, dur]` by the same rect math `seekAt` uses. The three mask kinds share ONE arm because the payload string IS the `EffectKind`, so the kind needs no mapping table; the text arm is a PREFIX test rather than four equality arms for the same economy, because the four text pills differ only in which kind they seed. Both use `dur_ms` 3000 against everything else's 2000, for the reason `useTimelineActions.md` gives on `addMask` and `addText`. The payloads themselves come from `EffectPills`' `PILLS` table, whose `type` field is exactly what this switches on.
