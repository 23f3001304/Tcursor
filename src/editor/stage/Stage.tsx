...
export function Stage({ src, webcamSrc, track, layout, layoutPresets, layoutSegs, cameraMoves, zooms, zoomSettings, clicks, bgUrl, cursorSprites, cursorKinds, osCursorInVideo, cursor, effects, clickfx, audioSrc, muted, volume, timeMs, playing, moveMode, aimPoint, aimMode, camDraftRef, onTime, onDuration, onZoomAt, onAimAt, onRetryMedia }: {
  ...
  useEffect(() => { camDraftRef.current = null; dirtyRef.current = true; }, [timeMs]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Dragging the reticle re-aims continuously; every move is a normal `update_zoom`, so the undo
  // history's COALESCE_MS window folds the whole drag into one step (no new undo machinery).
  const onReticleDown = (e: React.PointerEvent) => {
    e.stopPropagation();
    setAimDrag(true);
    const move = (ev: PointerEvent) => { const t = targetUnderPointer(ev.clientX, ev.clientY); if (t) onAimAt(t[0], t[1]); };
    const up = () => { setAimDrag(false); window.removeEventListener("pointermove", move); window.removeEventListener("pointerup", up); };
  ...
  const reticle = aimPoint && !playing
    ? mapZoomTargetToCanvasPoint({ tx: aimPoint[0], ty: aimPoint[1], canvasW, canvasH, layout, cam: camAt(track, timeMs) })
    : null;
      ...
      <div className="e-stage" style={{ aspectRatio: `${canvasW} / ${canvasH}` }}>
        ...
        {moveMode && (
          <CamDragHandle layout={layout} layoutPresets={layoutPresets} layoutSegs={layoutSegs} cameraMoves={cameraMoves}
            timeMs={timeMs} canvasW={canvasW} canvasH={canvasH} canvasRef={canvas} camDraftRef={camDraftRef} dirtyRef={dirtyRef} />
        )}
