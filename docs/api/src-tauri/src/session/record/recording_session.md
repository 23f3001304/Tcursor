...
- `src-tauri/src/session/record/recorder.rs` - constructed and driven inside the `"video"` thread; `frame_timestamps` and `stop_and_finalize` called after the run loop.
