// Tests for the pure seams in `pipeline/mod.rs` (trim bounds, audio shift, webcam-outcome
// classification), in their own file so `mod.rs` stays under the line budget.
use super::{audio_shift_ms, trim_frame_bounds, webcam_warning};

/// The three webcam outcomes must read differently, because the FILE differs: no frames at all
/// means no camera panel anywhere; a failure part-way means the panel is still there, frozen on
/// the last decoded frame. Reporting the second as "exported without the camera panel" would
/// send the user looking for a missing panel that is on screen.
#[test]
fn webcam_warning_separates_zero_frames_from_a_partial_failure() {
    assert_eq!(webcam_warning(false, Some("boom"), 0), None, "no webcam file: never a warning");
    assert_eq!(webcam_warning(true, None, 120), None, "a healthy decode says nothing");
    let none = webcam_warning(true, None, 0).unwrap();
    assert!(none.contains("no frames") && none.contains("without the camera panel"), "{none}");
    let early = webcam_warning(true, Some("moov atom not found"), 0).unwrap();
    assert!(early.contains("before any frame") && early.contains("moov atom not found"), "{early}");
    let partial = webcam_warning(true, Some("read failed"), 431).unwrap();
    assert!(partial.contains("after 431 frames") && partial.contains("frozen"), "{partial}");
    assert!(!partial.contains("without the camera panel"), "a frozen panel is not an absent one: {partial}");
}
...
/// The A/V lead bug: at 30 fps a 1234 ms trim-in floors to frame 37 (1233 ms), so video
/// frame 0 shows 1233 ms of source. Shifting audio by the raw 1234 would put it 1 ms - up
/// to a full 33 ms frame - ahead of the picture for the whole export.
#[test]
fn audio_shift_uses_the_frame_floored_trim_in() {
    ...
    // Frame-aligned trim-ins are unchanged (q == trim_in_ms), as is the untrimmed case.
