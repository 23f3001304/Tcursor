...
pub fn ensure_thumbs(folder: String, count: u32) -> Result<Vec<String>, String> {
    ...
    if !dir.join("thumb_0001.jpg").exists() {
...
pub fn ensure_waveform(folder: String, which: String) -> Result<String, String> {
    ...
    let out = paths.folder.join(format!("wave_{which}.png"));
    if !out.exists() {
        let tmp = crate::win::proc::tmp_sibling(&out); // write then atomic-rename
        let status = ffcmd("ffmpeg")
            .args(["-v", "error", "-y", "-i"]).arg(&wav)
            .args(["-filter_complex", "showwavespic=s=1180x26:colors=#6b6b86", "-frames:v", "1"])
            .arg(&tmp)
            .status().map_err(|e| e.to_string())?;
...
/// `-itsoffset`/`-ss` alignment (see `export::audio_mux` + `exporter.rs` `shift()`), so it can
...
pub fn prewarm(folder: String) {
    let _ = crate::export::preview_track::ensure_proxy(folder.clone(), 720);
