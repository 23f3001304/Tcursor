...
pub fn hold_alpha(actions: &[ActionEvent], et: u32, fade_ms: u32) -> f32 {
    crate::export::fx::hold::hold_alpha(
        actions,
        et,
        fade_ms,
...
mod tests {
    ...
    fn a(t: u32, k: ActionKind) -> ActionEvent {
        ActionEvent { t, kind: k }
    }
    #[test]
    fn ramps_up_holds_then_ramps_down() {
        let acts = vec![
            a(1000, ActionKind::SpotlightHoldStart),
            a(2000, ActionKind::SpotlightHoldEnd),
        ];
        assert_eq!(hold_alpha(&acts, 900, 200), 0.0);
        assert!((hold_alpha(&acts, 1100, 200) - 0.5).abs() < 0.01);
        assert_eq!(hold_alpha(&acts, 1500, 200), 1.0);
        assert!((hold_alpha(&acts, 2100, 200) - 0.5).abs() < 0.01);
        assert_eq!(hold_alpha(&acts, 2300, 200), 0.0);
    }
    #[test]
