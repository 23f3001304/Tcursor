...
export function isPreset(t: BackgroundThumb, bg: BackgroundSettings): boolean {
  ...
  if (!g) return false;
  return (
    same(g.from, bg.gradient_from) &&
    same(g.to, bg.gradient_to) &&
    same(g.mid ?? null, bg.gradient_mid ?? null) &&
    g.angle_deg === bg.gradient_angle_deg
  );
}
...
export function GradientTab({
  ...
  if (thumbs === null) {
    return (
      <div className="e-grp e-secstack">
        <CategorySection id="bg.gradient.loading" label="Presets" count={0} defaultOpen>
          <TileGridSkeleton />
        </CategorySection>
      </div>
    );
  ...
  const apply = (id: string) => {
    ...
    setBg({
      ...
      gradient_mid: g.mid ?? null,
      gradient_to: g.to,
      gradient_angle_deg: g.angle_deg,
    });
  };
      ...
      <div className="e-grp e-secstack">
        {groups.map((g, i) => (
          <CategorySection
            ...
            selectedName={g.tiles.find((t) => t.id === selectedId)?.name ?? null}
            defaultOpen={i === open}
          >
      ...
      <div className="e-grp">
        ...
        <div className="e-colorrow">
          ...
          {mid && (
            <ColorInput
              value={mid}
              label="Middle"
              ariaLabel="Gradient middle color"
        ...
        <div className="e-field">
          <Slider
            ...
            max={360}
            step={5}
            value={Math.round(bg.gradient_angle_deg)}
            onChange={(v) => edit({ gradient_angle_deg: v })}
