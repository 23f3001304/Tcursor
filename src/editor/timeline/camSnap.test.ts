...
// Task 27: dragging a camera keyframe snaps its TIME (ms, never px - so the feel is identical at
// every timeline zoom) to a layout-segment edge or a sibling keyframe within CAM_SNAP_MS.
describe("snapKeyframeMs", () => {
  it("snaps to a layout-segment edge just inside the window", () => {
    expect(snapKeyframeMs(1930, [1000, 2000], [])).toBe(2000); // 70ms away
  ...
  it("picks the NEAREST candidate when several are in range", () => {
    expect(snapKeyframeMs(2050, [2000], [2060])).toBe(2060); // 10ms beats 50ms
