...
**The one renamed field (Batch 4 T9).** `onSplit: () => void timeline.splitAt()` sits after the `...timeline` spread and overrides nothing - `timeline` has no `onSplit` of its own, only `splitAt`. It is the one place this file breaks its own "spread, don't rename" rule, and only because `Transport`'s tool wants a plain, already-void click handler named for what it does in the bar, not the async verb the hook returns. `ShellProps` (`slotProps.ts`) and the `<Transport>` call in `ClassicShell.tsx` both had to grow the one matching field/line for this to reach the button; neither file's own doc page enumerates props at this level of detail (see `slotProps.md`'s "`~80 fields - see the source`" and `ClassicShell.md`'s composition-level prose), so this is the one place the wiring is written down.
