// Tests for win::sys::proc, split into their own file so proc.rs stays under the size limit.
use super::*;
...
fn preserve_corrupt_moves_the_bad_file_aside_and_leaves_original_bytes_intact() {
    ...
    assert!(!path.exists(), "the bad file must be moved OFF the original path");
