...
const PRESETS: { name: string; v: Pick<ZoomSettings, "target_scale" | "smoothness" | "hold_ms"> }[] = [
  ...
  { name: "Balanced", v: { target_scale: 2.2, smoothness: 0.1, hold_ms: 2200 } },
...
function activePreset(v: ZoomSettings): string {
  const p = PRESETS.find(
    (p) =>
      p.v.target_scale === v.target_scale && p.v.smoothness === v.smoothness && p.v.hold_ms === v.hold_ms,
  );
    ...
    <section className="sec">
      ...
      <div className="sf">
        <div className="sf-row">
          <span className="sf-label">Zoom on click</span>
      ...
      <Field label="Clicks to zoom">
        <div className="seg">
          {CLICKS.map((n) => (
            <button
              key={n}
              className={`seg-btn ${value.clicks === n ? "on" : ""}`}
              onClick={() => set("clicks", n)}
            >
              {n}
            </button>
          ))}
      ...
      <Field label="Feel">
        <div className="seg" style={{ flexWrap: "wrap" }}>
          {PRESETS.map((p) => (
            <button
              key={p.name}
              className={`seg-btn ${cur === p.name ? "on" : ""}`}
              onClick={() => onChange({ ...value, ...p.v })}
            >
              {p.name}
            </button>
          ))}
          <button className={`seg-btn ${cur === "Custom" ? "on" : ""}`} onClick={() => setAdv(true)}>
            Custom
          </button>
        </div>
      </Field>
      ...
      <div className="sf">
        <div className="sf-row">
          <span className="sf-label">Smart type</span>
          <Switch on={value.smart_hold} onChange={(v) => set("smart_hold", v)} />
        </div>
      </div>
      <div className="sf">
        <div className="sf-row">
          <span className="sf-label">Smart follow</span>
      ...
      <Advanced open={adv} onToggle={setAdv}>
        ...
        <Range
          ...
          value={value.camera_smoothing_ms}
          min={0}
          max={400}
          step={10}
          onChange={(v) => set("camera_smoothing_ms", Math.round(v))}
          fmt={(v) => (v === 0 ? "Off" : `${v} ms`)}
        />
        ...
        <Range
          label={value.smart_hold ? "Idle release" : "Hold"}
          value={value.hold_ms}
          min={600}
          max={5000}
          step={100}
          onChange={(v) => set("hold_ms", Math.round(v))}
          fmt={(v) => `${(v / 1000).toFixed(1)} s`}
        />
        <div className="sf">
          <div className="sf-row">
            <span className="sf-label">Shrink camera on zoom</span>
