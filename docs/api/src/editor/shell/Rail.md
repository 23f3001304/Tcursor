...
**Tooltips and hover (look pass, 2026-09-14).** The owner's note was that the rail's icons needed both. Each button is wrapped in `Tooltip` (`controls/Tooltip.tsx`), which fades its label in to the right of the rail after a 350ms hover dwell - and the plain `title` attribute stays on the button underneath it, so the native tooltip is still there as the fallback and for anything reading the accessibility tree. Hover itself now lands the icon on `--e-raised` at full `--e-fg` (`.e-ric:not(.on):hover`, `stage/stage.css`), which is where the rail's rules moved with the rest of the centre column's. The `:not(.on)` guard is deliberate: the active button already sits on its own pill, and on the AI tab that pill is violet-tinted, so a grey fill under it would only muddy the color.
