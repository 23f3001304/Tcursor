# src/editor/timeline/model/layoutTrack.ts
...
Map progress `p` (clamped to `[0,1]`) through the curve named by an easing wire-name. **This is the only place easing is evaluated in TypeScript** - `layoutAt` below and `camMoveAt` (`stage/camera/cameraMoves.ts`) both call it, and it is exported for exactly that reason.
...
- `linear` - identity.
- `spring` / `spring(stiffness,damping[,mass])` - a real damped harmonic oscillator, delegated to `spring` (`src/shared/math/spring.ts`), which mirrors `export::spring`. `springOf` resolves both spellings - the bare word means `SPRING_DEFAULT` - and it is checked BEFORE the named curves so a parameterised spring cannot fall through to smooth. Underdamped params overshoot past 1 and ring down; critical and overdamped ones are monotone.
- `keys(t v in_dx in_dy out_dx out_dy mode, ...)` - a hand-drawn curve from the motion editor, delegated to `parseKeys`/`evalKeys` (`src/editor/motion/keys.ts`), which mirror `export::keys`. Checked before the named curves for the same reason the spring is: a curve the graph wrote must never fall through to smooth. Its VALUE is not clamped - a key may sit below 0 or above 1 and the ramp anticipates or overshoots - so this is the one arm that can return outside `[0,1]` other than the spring. A malformed `keys(...)` parses to `null` and falls through to the smooth default below, exactly as `valid_easing` would coerce it.
- `ease_in` / `ease_out` / `ease_in_out` - quadratic accelerate / decelerate / symmetric.
- `cubic(x1,y1,x2,y2)` - a custom bezier, delegated to `evalCubic` (`src/shared/math/cubicBezier.ts`), which mirrors `export::cubic::eval`.
...
`toKeys` (`motion/keys.ts`) converts the arms above that have an exact bezier equivalent into keys form, so the graph editor can start a drag from any of them; it deliberately accepts only the wire names THIS function accepts, so the two can never disagree about what a string means.
...
`lerpRect` mirrors the Rust `lp`: rect, radius, alpha and ring width all cross-fade continuously, but `ring_color` SNAPS to the destination's colour at the midpoint - an RGB lerp mid-transition looks muddy and the export never blends it either. It builds on `lerpN` - the plain linear interpolation, imported from `../stage/canvas/sourceSpans.ts` (documented there) rather than redefined here, since `spanAt`'s `fit` cross-fade needs the identical one-liner.
...
- `src/editor/hooks/stage/useCompositeLoop.ts` - resolves the frame layout every composited frame.
- `src/editor/stage/camera/CamDragHandle.tsx` - derives the PiP rect the Move-mode handle sits on, so the handle matches the drawn frame.

Both are EXPORTED as of T34 L3: stage arrange mode needs a segment's STEADY-STATE panels - what it resolves to outside any transition - to frame and to start a drag from, and re-deriving that beside `layoutAt` would be a second place for the per-segment-override-vs-preset rule to live. `useArrangeDrag` and `LayoutInspector` both call this one - as of T34 L4, so does `src/editor/timeline/lanes/LayoutLane.tsx`'s `useLayoutLaneRegions`, which attaches each layout pill's resolved panels for its thumbnail (`arrThumb.ts`/`LayoutLane.tsx`).
