...
1. **Background decode.** The export background (a data URL) is decoded once per change into the `<img>` the canvas draws, marking the frame dirty when it lands. It writes into `bgRef`'s `StageBgState` (`stage/stageBg.ts`) - the single object holding everything the background draw needs, which is what let `Stage`, `useCompositeLoop` and `drawPreview` gain video backgrounds without gaining a prop.
