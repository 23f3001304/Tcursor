# src-tauri/src/export/fx/spot/spotlight_sim.rs

The stateful spotlight region resolver. `fx_state.rs` builds the per-frame state and is otherwise stateless; this is the one part of it that remembers the previous frame, so it lives here with the rest of the spotlight. Re-exported as `fx_state::SpotlightSim` so callers name it where they use it; `region_alpha` stays internal (only `SpotlightSim::resolve` and this file's own tests use it directly).
