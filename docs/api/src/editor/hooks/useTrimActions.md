...
- `timeMs: number` - the current playhead position; each trim action snaps the corresponding trim point to `Math.round(timeMs)`.
...
Fresh closures each render, computed over the current `doc.trim`/`timeMs`/`dur` - no React state of its own.
