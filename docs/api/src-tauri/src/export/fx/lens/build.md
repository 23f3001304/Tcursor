# src-tauri/src/export/fx/lens/build.rs

Where the glass shapes GO this frame. `lens/mod.rs` owns the types and the curves; placement is its own responsibility and lives here. It is kept out of `cursorset.rs` because the FX pass needs the answer BEFORE the cursor blit runs.
...
**Plain-OS gets nothing.** `System` on a recording that baked no cursor promises a plain arrow, not "Enhanced minus polish" - the same rule `cursorset::draw` already applies to the bounce and the trail, and `stageCursor` (`src/editor/stage/Stage.tsx`) mirrors on the preview side.

**Squash, and why the sprite lens usually has none.** The sprite lens's box already carries `cursordraw::bounce_scale`'s click dip, so applying `lens::squash_at` on top of it would dip twice. Its own squash therefore only applies when `click_bounce` is off, which is what keeps the glass reacting to a click for a user who turned the bounce off. The back has no sprite to follow, so it always squashes.
...
The sprite lens itself: the already-interpolated box, the rotation in radians, and the mask - the settled kind's `Arc` clone, or both kinds' blended (`lens::morph_mask`) while a state change is still easing.
...
- `src-tauri/src/export/cursor/pack/cursorset.rs` - `draw`, on the glass-pack branch.
