...
/** The contract with Rust's `export::spring_tests::parity_table_matches_the_ts_mirror` - the same
 *  numbers at p = 0.1 .. 0.9. If these move, that table has to move with them, or the editor
 *  preview would draw a different curve from the one the export renders. */
const PARITY: [number, number, number, number[]][] = [
  [170, 26, 1, [0.154220, 0.405341, 0.618152, 0.768274, 0.865271, 0.924838, 0.960285, 0.980989, 0.992996]],
  [300, 10, 1, [1.216900, 1.107563, 0.874016, 1.055725, 0.994093, 0.989035, 1.007699, 0.996468, 0.999043]],
  [170, 60, 1, [0.471265, 0.735163, 0.867478, 0.933845, 0.967160, 0.983910, 0.992357, 0.996643, 0.998844]],
  [100, 10, 1, [0.547465, 1.085405, 1.145100, 1.031899, 0.975458, 0.983249, 1.000326, 1.004673, 1.002020]],
];
...
describe("spring", () => {
  ...
  it("has exact endpoints on every branch", () => {
    for (const [k, c, m] of [[170, 26, 1], [300, 10, 1], [170, 60, 1], [2000, 0, 1], [1, 200, 10]]) {
  ...
  it("overshoots more the lower the damping ratio, and not at all past critical", () => {
    ...
    expect(peak(170, 60)).toBeCloseTo(1, 4); // overdamped: monotone, never past the target
  });

  it("is monotone when critically damped or overdamped", () => {
    for (const [k, c] of [[170, 26.077], [170, 60], [1, 200]]) {
      const v = sweep(k, c, 1);
  ...
  it("depends on the damping ratio alone, so the same zeta is the same curve", () => {
    // The documented consequence of remapping onto the spring's own settle time: w0 cancels.
    for (let i = 1; i < 10; i++) {
...
describe("parseSpring / formatSpring", () => {
  ...
  it("clamps out-of-range values and rejects malformed input", () => {
    ...
    for (const bad of ["spring(170)", "spring(1,2,3,4)", "spring()", "spring(a,b)", "spring(1,2",
      "cubic(0,0,1,1)", "spring", "", null, undefined]) {
