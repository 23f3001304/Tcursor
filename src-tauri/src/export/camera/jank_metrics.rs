...
pub fn series(r: &Run) -> Vec<Series> {
    ...
    let vels = vec![
        ...
        (
            "scale",
            px(&r.scale, &|i| FW as f32 / (2.0 * s[i].max(0.01))),
        ),
    ];
    vels.into_iter()
        .map(|(name, vel)| {
...
pub fn max_abs(v: &[f32]) -> (usize, f32) {
    v.iter().enumerate().fold(
        (0, 0.0),
        |(bi, bv), (i, x)| if x.abs() > bv { (i, x.abs()) } else { (bi, bv) },
    )
}
...
pub fn top_spikes(r: &Run, n: usize) -> Vec<Spike> {
    ...
    for sp in all {
        if out
            .iter()
            .any(|o| o.comp == sp.comp && o.i.abs_diff(sp.i) <= 2)
        {
            continue;
        }
        out.push(sp);
...
pub fn classify(r: &Run, i: usize) -> String {
    let t = r.t[i];
    let near = |x: u32| t.abs_diff(x) <= 34;
    ...
    for (idx, (s, e)) in [R1, R2, R3].iter().enumerate() {
        if r.driver[i] != idx as i32 {
            continue;
        }
        if near(s + ZI) {
            return "H1 ramp-in -> hold".into();
        }
        if near(e - ZO) {
            return "H5 hold -> ramp-out (centre freezes)".into();
        }
        if near(*s) {
            return "H2 region start (fresh ramp from rest)".into();
        }
        if near(*e) {
            return "H6/H5 region end".into();
        }
    }
    if i > 0 && r.clamped[i] != r.clamped[i - 1] {
        return format!(
            "H6 clamp {}",
            if r.clamped[i] { "engaged" } else { "released" }
        );
    }
    if r.clamped[i] {
        return "H6 clamp riding the frame edge".into();
    }
...
pub fn print_spikes(r: &Run, n: usize) {
    ...
    println!(
        "  {:>5} {:>8} {:>6} {:>10} {:>10} {:>10}  {}",
        "#", "t(ms)", "chan", "v[i-1]px", "v[i]px", "dv px/f2", "classification"
    );
    for (k, sp) in top_spikes(r, n).iter().enumerate() {
        println!(
            "  {:>5} {:>8} {:>6} {:>10.3} {:>10.3} {:>10.3}  {}",
            k + 1,
            sp.t,
            sp.comp,
            sp.v0,
