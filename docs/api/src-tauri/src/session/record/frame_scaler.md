# src-tauri/src/session/record/frame_scaler.rs
...
Split from `gpu_frames.rs` (the WGC callback, already at the 200-line cap) the way `pause_clock.rs` and `close_guard.rs` were, and from `frame_fit.rs` because the geometry there is pure and unit-tested while nothing here can run without a live GPU capture.
