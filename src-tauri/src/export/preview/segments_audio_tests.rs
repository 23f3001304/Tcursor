...
fn part(path: &str, delay_ms: u64) -> MicPart {
    MicPart {
...
fn temp_project(tag: &str) -> ProjectPaths {
    let folder =
        std::env::temp_dir().join(format!("tcursor_micmerge_{}_{tag}", std::process::id()));
...
fn write_wav(path: &Path, ms: u32, rate: u32) {
    let mut w =
        WavWriter::create(path.to_str().expect("utf8 temp path"), rate, 1).expect("create wav");
    let n = (rate as u64 * ms as u64 / 1000) as usize;
    w.write(
        &(0..n)
            .map(|i| ((i % 96) as i16 - 48) * 300)
            .collect::<Vec<i16>>(),
    );
...
fn two_segments_mix_with_the_second_delayed_by_its_gap() {
    let args = merge_args(
        &[part("mic.wav", 0), part("mic_2.wav", 4200)],
        48000,
        1,
        Path::new("out.wav"),
    );
    ...
    assert_eq!(
        &a[..4],
        ["-i", "mic.wav", "-i", "mic_2.wav"].map(String::from)
    );
    assert_eq!(
        filter_of(&args),
...
fn delays_are_measured_from_the_first_segments_own_start() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            MicPart {
                path: paths.folder.join("mic_3.wav"),
                delay_ms: 6000
            }
        ]
    );
...
fn a_take_that_started_muted_measures_from_the_videos_first_frame() {
    ...
    assert_eq!(
        parts_of(&paths, &sync),
        vec![MicPart {
            path: paths.folder.join("mic_2.wav"),
            delay_ms: 4000
        }]
    );
    let _ = std::fs::remove_dir_all(&paths.folder);
...
fn two_generated_wavs_merge_into_one_track_with_the_gap_between_them() {
    ...
    assert!(
        (dur - 0.250).abs() < 0.005,
        "merged mic.wav is {dur}s, expected 0.250"
    );
    assert!(
        paths.folder.join("segments").join("mic.wav").exists(),
        "the first segment is kept"
    );
    assert!(
        paths.folder.join("segments").join("mic_2.wav").exists(),
