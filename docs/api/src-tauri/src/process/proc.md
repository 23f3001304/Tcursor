...
- `src-tauri/src/process/proc.rs` (`preserve_corrupt`) - computes the destination
...
- `src-tauri/src/process/proc.rs` (`init_ffmpeg`) - called once with the winning candidate directory
...
Pure helper that returns the first directory in `candidates` that contains a file named `name`.

### Inputs

- `candidates: &'a [PathBuf]` - candidate directories in priority order (most-preferred first). *Why a slice:* `ffmpeg_candidates` builds a `Vec`; a slice reference avoids an allocation.
...
`Option<&'a PathBuf>` - a reference to the first element of `candidates` for which `d.join(name).exists()` is true, or `None` if no candidate matches.

### Implementation

1. `candidates.iter().find(|d| d.join(name).exists())` - linear scan, short-circuits on first hit.
...
3. Call `choose_dir(&candidates, &name)` to find the winner.
...
`Command` pre-configured with the resolved binary path and, on Windows, the `CREATE_NO_WINDOW` creation flag (imported from `windows::Win32::System::Threading`, never re-declared). The caller adds arguments and spawns normally.
...
3. (Windows only, via the private `ffcmd_prio` helper) `c.creation_flags(CREATE_NO_WINDOW.0 | <priority>)` through the `CommandExt` trait - `ffcmd` passes `background: false`, so the priority term is `0`. *Why the no-window flag:* without it, each ffmpeg invocation momentarily flashes a black console window on the user's desktop.
...
Identical to `ffcmd` but adds `BELOW_NORMAL_PRIORITY_CLASS` (also imported from `windows::Win32::System::Threading`) to the Windows creation flags, so the spawned ffmpeg runs at below-normal process priority.
