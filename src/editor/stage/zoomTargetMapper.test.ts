...
// The canvas is DISPLAYED at half its backing-store size and offset in the viewport, so a
// round-trip that ignored the bounding rect would drift.
const fakeCanvas = {
  width: CANVAS_W, height: CANVAS_H,
  getBoundingClientRect: () => ({ left: 40, top: 24, width: CANVAS_W / 2, height: CANVAS_H / 2 }),
};
const LAYOUT: PreviewLayout = { screen: [0.1, 0.05, 0.7, 0.8], radius: 0, cam: null, canvas: [CANVAS_W, CANVAS_H] };

/** Client coords of a canvas fraction, the inverse of the rect math in the mapper. */
const clientOf = ([fx, fy]: [number, number]) => ({ clientX: 40 + fx * (CANVAS_W / 2), clientY: 24 + fy * (CANVAS_H / 2) });
...
describe("mapZoomTargetToCanvasPoint (T31 stage reticle)", () => {
  it("is the exact forward of mapCanvasClickToZoomTarget at scale 1", () => {
    ...
    for (const t of [[0, 0], [0.5, 0.5], [0.25, 0.8], [1, 1]] as [number, number][]) {
      const pt = mapZoomTargetToCanvasPoint({ tx: t[0], ty: t[1], canvasW: CANVAS_W, canvasH: CANVAS_H, layout: LAYOUT, cam });
      const back = mapCanvasClickToZoomTarget({ ...clientOf(pt), canvasElement: fakeCanvas, layout: LAYOUT, cam });
  ...
  it("round-trips through a live zoom crop (scale 2.5, off-centre camera)", () => {
    ...
    for (const t of [[0.4, 0.6], [0.35, 0.55], [0.5, 0.7]] as [number, number][]) {
      const pt = mapZoomTargetToCanvasPoint({ tx: t[0], ty: t[1], canvasW: CANVAS_W, canvasH: CANVAS_H, layout: LAYOUT, cam });
      const back = mapCanvasClickToZoomTarget({ ...clientOf(pt), canvasElement: fakeCanvas, layout: LAYOUT, cam });
      expect(back).not.toBeNull();
  ...
  it("puts the camera's own aim point at the centre of the canvas when zoomed in", () => {
    // A zoom centred on its target crops around it, so the target renders dead centre.
    const [fx, fy] = mapZoomTargetToCanvasPoint({
      tx: 0.5, ty: 0.5, canvasW: CANVAS_W, canvasH: CANVAS_H, layout: LAYOUT, cam: { scale: 2, cx: 0.5, cy: 0.5 },
    });
  ...
  it("reports fractions outside 0..1 for an aim point cropped out of frame", () => {
    const [fx] = mapZoomTargetToCanvasPoint({
      tx: 0.02, ty: 0.5, canvasW: CANVAS_W, canvasH: CANVAS_H, layout: LAYOUT, cam: { scale: 3, cx: 0.9, cy: 0.5 },
    });
