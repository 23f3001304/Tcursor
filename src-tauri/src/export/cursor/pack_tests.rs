...
#[test]
fn exactly_one_shipped_pack_is_a_lens_and_the_default_is_never_one() {
    // `material: "glass"` is what makes the FX pass refract through a pack's sprites instead of
    // blitting them. Getting it wrong is invisible in the picker and obvious in the export, so the
    // manifest is pinned here - by COUNT rather than by id, so renaming the pack's folder is a
    // one-file change. The embedded set has no manifest at all and can never pick a material up.
    assert_eq!(material(DEFAULT_PACK_ID), None);
    assert!(!is_glass(DEFAULT_PACK_ID) && !is_glass("") && !is_glass("no_such_pack"));
    let shipped = list_packs(None);
    let glass: Vec<&str> = shipped.iter().filter(|p| is_glass(&p.id)).map(|p| p.id.as_str()).collect();
    assert_eq!(glass.len(), 1, "exactly one shipped pack declares material: glass, got {glass:?}");
    assert_eq!(shipped.iter().find(|p| is_glass(&p.id)).map(|p| p.category.as_str()),
        Some("Glass and glow"), "and it is listed in the glass section");
    // The listing carries the same answer to the frontend as `pack::material` gives the renderer.
    for p in &shipped {
        assert_eq!(p.material, material(&p.id), "{} on the wire vs on disk", p.id);
    }
}
