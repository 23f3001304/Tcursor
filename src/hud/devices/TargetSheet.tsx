...
export function TargetSheet({
  ...
  const rows = targets
    .map((t, i) => ({ t, meta: parseTarget(t, i) }))
    .filter((r) => !isOwnProcessWindow(r.t));
  ...
  const row = (r: (typeof rows)[number], label: string, sub: string | null, glyph: React.ReactNode) => (
    <button
      ...
      type="button"
      role="option"
      aria-selected={r.t.id === value}
      className={`dd-item tgt${r.t.id === value ? " sel" : ""}`}
      onClick={() => onPick(r.t.id)}
    >
  ...
  return (
    <div className="sheet">
      <div className="sheet-head">
        <button type="button" className="winbtn" title="Back" aria-label="Back" onClick={onBack}>
          <Back />
        </button>
      ...
      <div className="sheet-list" role="listbox" aria-label="What to record">
        ...
        {screens.map((r) =>
          row(
            r,
            r.meta.title.replace(/^Display \d+: /, ""),
            [r.meta.resolution?.replace("x", " by "), r.meta.primary ? "Primary" : null]
              .filter(Boolean)
              .join(" \u00b7 ") || null,
            <Monitor />,
          ),
