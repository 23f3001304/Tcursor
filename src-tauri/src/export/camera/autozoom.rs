...
/// Latest held time: from `start`, repeatedly jump to the next activity within
/// `idle_ms`. `acts` must be sorted ascending. Returns the last activity time reached.
fn extend_hold(start: u32, acts: &[u32], idle_ms: u32) -> u32 {
    let mut last = start;
    for &t in acts {
        if t < last { continue; }
        if t.saturating_sub(last) <= idle_ms { last = t; } else { break; }
    }
    last
}
...
pub fn generate(events: &[MouseEvent], screen: &ScreenInfo, cfg: &ZoomConfig, typing: &[u32], smart: bool) -> Vec<ZoomRegion> {
    ...
    while i < downs.len() {
            ...
            && downs[i + need - 1].t.saturating_sub(downs[i].t) <= cfg.merge_window_ms
        {
            ...
            // Activity after the cluster that can extend the hold: subsequent clicks, plus
            // typing keystrokes when smart. Merged + sorted, then chained by idle gaps.
            let mut acts: Vec<u32> = downs[i + need..].iter().map(|d| d.t).collect();
            if smart { acts.extend(typing.iter().copied()); }
            acts.retain(|&t| t >= cluster_last);
            acts.sort_unstable();
            let last_t = extend_hold(cluster_last, &acts, cfg.idle_release_ms);
            let mut j = i + need;
            while j < downs.len() && downs[j].t <= last_t { j += 1; }
...
mod tests {
    ...
    fn typing_extends_the_hold_when_smart() {
        ...
        let typing = vec![1500u32, 3000, 4500]; // keystrokes after the click, each within idle_release
        let r = generate(&ev, &s, &cfg, &typing, true);
        assert_eq!(r.len(), 1);
        // hold extends to the last keystroke (4500), not just the click (1000)
    ...
    fn typing_ignored_when_not_smart() {
        ...
        assert_eq!(r[0].end_ms, 1000 + cfg.idle_release_ms + cfg.zoom_out_ms); // click-only
    }
}
