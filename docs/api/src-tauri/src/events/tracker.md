# src-tauri/src/events/tracker.rs
...
- `src-tauri/src/session/recorder.rs` - started at recording begin; `stop` called at recording end to collect events
- `src-tauri/src/session/recorder_threads.rs` - orchestrates tracker lifecycle alongside `CursorTypeTracker`
