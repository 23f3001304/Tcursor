...
- `app: AppHandle` - Tauri app handle used to emit events, and to resolve the `Arc<Platform>` whose `system` port is handed to `exporter::export`. *Why:* cloned once so both the progress closure and the final outcome can emit independently without a move conflict. *Why the platform is resolved BEFORE the thread spawns:* `tauri::State` borrows the app, so it cannot cross a `move` closure; the `Arc<Platform>` clone can, and does.
...
- The display-refresh query (capped at 60) that used to live here now lives inside `exporter::export` itself (as `capture_fps`), since it also feeds `Fps::Source`'s fallback. What `run_export` does supply, since Batch D, is the `SystemPort` that query goes through: this function is the composition root for the export, because it is the last place with an `AppHandle`.
