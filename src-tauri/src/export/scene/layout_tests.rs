...
fn switch_transitions_then_settles() {
    ...
    let (lo, hi) = (
        camera.screen.rect.w.min(screen.screen.rect.w),
        camera.screen.rect.w.max(screen.screen.rect.w),
    );
...
fn per_segment_transition_feel_drives_the_crossfade() {
    let app = AppearanceSettings::default();
    let segs = vec![
        seg("l0", 0, 1000, "screen", 0),
        seg("l1", 1000, 4000, "camera", 400),
    ];
    let track = LayoutTrack::from_segs(&segs, &app, 3840, 2160, 1920, 1080);
    assert_ne!(
        track.scene_at(1200),
        scene_of(&app, LayoutId::Camera),
        "at 200/400ms the fade should not have reached camera yet"
    );
    let snap = vec![
        seg("l0", 0, 1000, "screen", 0),
        seg("l1", 1000, 4000, "camera", 0),
    ];
    assert_eq!(
        LayoutTrack::from_segs(&snap, &app, 3840, 2160, 1920, 1080).scene_at(1000),
        scene_of(&app, LayoutId::Camera)
    );
}
...
fn a_gap_between_segments_falls_back_to_screen() {
    ...
    let segs = vec![
        ...
        seg("l1", 2000, 3000, "presenter", 0),
    ];
    let track = LayoutTrack::from_segs(&segs, &app, 3840, 2160, 1920, 1080);
    assert_eq!(track.scene_at(500), scene_of(&app, LayoutId::Camera));
    assert_eq!(track.scene_at(1500), screen);
    assert_eq!(track.scene_at(3500), screen);
    assert_eq!(track.scene_at(2500), scene_of(&app, LayoutId::Presenter));
}
...
fn default_zero_exit_is_the_historical_hard_cut() {
    ...
    assert_eq!(
        track.scene_at(999),
        scene_of(&app, LayoutId::Camera),
        "last active ms is still fully camera"
    );
    assert_eq!(
        track.scene_at(1000),
        scene_of(&app, LayoutId::Screen),
        "and it cuts to the gap default"
    );
}
...
fn exit_blend_completes_exactly_at_end_ms() {
    ...
    assert_eq!(
        track.scene_at(600),
        camera,
        "the window opens AT f=0, i.e. still fully camera"
    );
    let mid = track.scene_at(800);
    assert!(
        rect_delta(&mid, &camera) > 1.0 && rect_delta(&mid, &screen) > 1.0,
        "mid-exit is between the two"
    );
    assert!(
        rect_delta(&track.scene_at(999), &track.scene_at(1000)) < 0.05,
...
fn a_gapless_successors_entry_wins_the_overlap() {
    ...
    let camera = scene_of(&app, LayoutId::Camera);
    let segs = vec![
        with_exit(seg("l0", 0, 1000, "camera", 0), 400),
        seg("l1", 1000, 2000, "presenter", 400),
    ];
    let track = LayoutTrack::from_segs(&segs, &app, 3840, 2160, 1920, 1080);
    for t in [600, 800, 999] {
        assert_eq!(track.scene_at(t), camera, "exit suppressed at {t}");
    }
    assert_eq!(
        track.scene_at(1000),
        camera,
        "the successor's entry starts FROM camera - seamless"
    );
    assert_eq!(track.scene_at(1400), scene_of(&app, LayoutId::Presenter));
    let hard = vec![
        with_exit(seg("l0", 0, 1000, "camera", 0), 400),
        seg("l1", 1000, 2000, "presenter", 0),
    ];
    let track = LayoutTrack::from_segs(&hard, &app, 3840, 2160, 1920, 1080);
    assert_ne!(
        track.scene_at(800),
        camera,
        "exit runs into a hard-cutting successor"
    );
...
fn anchor_frame_follows_the_panel_frame_by_frame() {
    ...
    let panel = |x: f32, y: f32, w: f32, h: f32| Panel {
        rect: RectF { x, y, w, h },
        radius: 0.0,
        alpha: 1.0,
        ring_px: 0.0,
    ...
    let scene = |r: RectF| Scene {
        screen: panel(r.x, r.y, r.w, r.h),
        camera: panel(0.0, 0.0, 1.0, 1.0),
        src: RectF {
            x: 0.0,
            y: 0.0,
            w: 1920.0,
    ...
    assert_eq!(
        out[0].anchor,
        FramePoint { x: 960, y: 540 },
        "a full-frame panel is the identity"
    );
    anchor_frame(
        &raw,
        &scene(RectF {
    ...
    assert_eq!(
        out[0].anchor,
        FramePoint { x: 580, y: 320 },
        "the same aim, at the centre of the half-size panel"
    );
    assert_eq!(
        raw[0].anchor,
        FramePoint { x: 960, y: 540 },
        "the raw anchor is never touched"
    );
}
