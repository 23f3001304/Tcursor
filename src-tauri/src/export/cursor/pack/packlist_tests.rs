...
fn the_embedded_default_pack_lists_first_with_a_real_folder() {
    ...
    let d = &packs[0];
    assert_eq!(
        (d.id.as_str(), d.name.as_str(), d.builtin),
        (DEFAULT_PACK_ID, "Default", true)
    );
    assert!(
        !d.dir.is_empty(),
        "the embedded pack must resolve to a folder"
    );
...
fn the_default_pack_resolves_all_nine_kinds_through_the_alias_map() {
    ...
    for &(kind, ..) in SPRITES {
        ...
        let file = d
            .files
            .get(&name)
            .unwrap_or_else(|| panic!("no file mapped for {name}"));
        assert!(
            Path::new(&d.dir).join(file).is_file(),
...
fn a_still_busy_state_shows_the_arrow_in_the_grid_exactly_as_it_renders() {
    ...
    assert!(
        d.busy.is_none(),
        "the embedded pack declares no busy animation"
    );
    assert_eq!(
        d.files[&kind_name(CursorType::Busy)],
        d.files[&kind_name(CursorType::Arrow)]
    );
}
...
fn a_v2_pack_keeps_its_own_busy_file_and_its_animation() {
    ...
    let cat = packs
        .iter()
        .find(|p| p.id == "cat")
        .expect("the Cat pack should be listed");
    assert_eq!(
        cat.busy.map(|b| (b.anim, b.frames)),
        Some((BusyAnim::Pulse, 0))
    );
    assert_eq!(
        cat.files[&kind_name(CursorType::Busy)],
        "busy.png",
        "an animated pack keeps its own"
    );
...
fn list_packs_puts_the_embedded_default_first_then_every_bundled_pack() {
    ...
    assert!(
        shipped.len() >= 15,
        "expected the bundled packs, got {:?}",
        packs.len()
    );
}
...
fn every_bundled_pack_manifest_declares_a_known_category() {
    ...
    for entry in std::fs::read_dir(&dir).expect("the bundled cursorpacks folder") {
        ...
        assert!(
            KNOWN_CATEGORIES.contains(&category.as_str()) && category != IMPORTED_CATEGORY,
    ...
    assert!(
        seen >= 15,
        "expected the fifteen bundled packs, found {seen}"
    );
}
...
fn a_pack_with_no_category_of_its_own_lists_under_imported() {
    ...
    assert_eq!(
        category_or_imported(Some(" Playful ".to_string())),
        "Playful"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
