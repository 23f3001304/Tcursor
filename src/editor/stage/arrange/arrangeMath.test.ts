...
import type { PanelPose } from "../../../lib/edit";
import { movedPose, poseOfRect, rectAspect, resizedPose, MAX_SIZE, MIN_SIZE, SNAP_PX, SNAP_TARGETS, type Rect, type SnapTol } from "./arrangeMath";

const OW = 1920, OH = 1080;
// The stage displayed 1:1 with its backing store, so "8 stage-px" and "8 output-px" coincide and
// every threshold assertion below can be written in output pixels. `null` is Alt.
const TOL: SnapTol = [SNAP_PX / OW, SNAP_PX / OH];
// A 16:9 panel dead-centre in a 16:9 frame: half the frame's width AND height, so the pixel aspect
// is the frame's own (16/9) and every anchoring assertion below reads in round numbers.
...
describe("poseOfRect / rectAspect", () => {
  it("reads a rect's centre and height, and its PIXEL aspect (w/h are fractions of different axes)", () => {
    ...
    expect(rectAspect(CAM, OW, OH)).toBeCloseTo(16 / 9, 9); // 0.2*1920 / 0.2*1080
  });
});
...
describe("movedPose", () => {
  ...
  it("snaps the CENTRE onto the frame centre and reports the guide", () => {
    const out = movedPose(pose(0.5, 0.5, 0.3), [0.003, 0], ASPECT, OW, OH, TOL); // 5.8px off
  ...
  it("snaps an EDGE, not just the centre - the left edge lands on the 4.5% safe margin", () => {
    // A 0.2-high 16:9 panel is 0.3556 wide; put its left edge ~3px right of the safe margin.
  ...
  it("drops a guide the CLAMP pulled the panel off (guides re-derived from the final pose)", () => {
    // Pushed hard right: cx clamps to 1, then the left edge snaps onto the 0.955 safe margin - a
    // shift that would put the centre at 1.002, which the clamp takes straight back to 1. The
    // panel therefore does NOT end up on the line, so no guide may be drawn through it.
...
describe("resizedPose", () => {
  ...
  it("anchors the OPPOSITE corner: dragging bottom-right keeps the top-left pinned", () => {
    const out = at("br", [1, 1]); // 1.5x on the diagonal
  ...
  it("never stretches: width stays height x the panel's own aspect for an off-diagonal drag", () => {
    const out = at("tr", [0.95, 0.4]); // nowhere near the diagonal
    const wPx = (out.pose.size * OH * ASPECT), hPx = out.pose.size * OH;
  ...
  it("clamps the size to the op's own range in both directions", () => {
    ...
    expect(at("br", [0.25, 0.25]).pose.size).toBe(MIN_SIZE); // collapsed onto the anchor
    expect(at("br", [-5, -5]).pose.size).toBe(MIN_SIZE); // dragged through the anchor
  });

  it("snaps the DRAGGED edge onto a target (the anchor stays put) and reports the guide", () => {
    // 455px tall is 5px past the height whose bottom edge sits on the lower third.
    ...
    expect(out.pose.cy - out.pose.size / 2).toBeCloseTo(0.25, 9); // anchor unmoved
  });
