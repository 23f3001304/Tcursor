...
fn gradient(from: Rgb, to: Rgb, angle_deg: f32) -> Background {
    Background::Gradient { from, mid: None, to, angle_deg }
}
...
fn a_middle_stop_lands_at_the_ramp_midpoint() {
    let (w, h) = (101u32, 1u32);
    let mid = Rgb { r: 200, g: 40, b: 10 };
    let buf = render(&Background::Gradient { from: BLACK, mid: Some(mid), to: WHITE, angle_deg: 0.0 }, w, h);
    let px = |x: u32, c: usize| buf[(x * 4) as usize + c] as i32;
    assert_eq!((px(50, 2), px(50, 1), px(50, 0)), (200, 40, 10), "t=0.5 must be exactly the middle stop");
    assert!(px(25, 2) > 90 && px(25, 2) < 110, "the first half ramps black -> mid, got {}", px(25, 2));
    assert!(px(75, 1) > 140 && px(75, 1) < 155, "the second half ramps mid -> white, got {}", px(75, 1));
}

#[test]
fn no_middle_stop_is_the_two_stop_ramp_unchanged() {
    let two = render(&gradient(BLACK, WHITE, 47.0), 32, 18);
    let explicit_none = render(&Background::Gradient { from: BLACK, mid: None, to: WHITE, angle_deg: 47.0 }, 32, 18);
    assert_eq!(two, explicit_none, "`mid: None` must be bit-identical to the pre-middle-stop ramp");
}

// `build` tests only exercise Solid/Gradient (pure Rust, deterministic). BOTH `Mesh` paths - the
// bundled wallpaper library and the legacy `bg.jpg` - shell out to ffmpeg, so they are covered
// where that decode itself is (`pipeline::ffio_tests`, which skips without ffmpeg) plus
// `wallpapers`' own id-lookup tests, which are what decide between the two.
#[test]
fn build_solid_matches_direct_render() {
    let s = BackgroundSettings { kind: BackgroundKind::Solid, solid: [10, 20, 30], ..Default::default() };
    let buf = build(&s, &[], 2, 2, Path::new(".")); // mesh bytes + project dir irrelevant for Solid
...
fn build_zero_blur_is_a_no_op() {
    ...
    let direct = render(&gradient(rgb(s.gradient_from), rgb(s.gradient_to), s.gradient_angle_deg), 16, 16);
    assert_eq!(buf, direct);
}

#[test]
fn build_passes_the_middle_stop_through_to_the_renderer() {
    let s = BackgroundSettings { kind: BackgroundKind::Gradient, gradient_from: [0, 0, 0],
        gradient_mid: Some([9, 9, 9]), gradient_to: [255, 255, 255], gradient_angle_deg: 0.0, ..Default::default() };
    let buf = build(&s, &[], 101, 1, Path::new("."));
    assert_eq!(buf[(50 * 4) as usize], 9, "the settings' middle stop must reach `render`");
}
...
/// A fresh empty scratch folder (see `settings::bg_asset_tests` for the same three lines and why
/// there is no shared test-util crate).
fn tempdir() -> std::path::PathBuf {
    use std::sync::atomic::{AtomicU64, Ordering};
    static SEQ: AtomicU64 = AtomicU64::new(0);
    let p = std::env::temp_dir().join(format!("tcursor_bgs_{}_{}", std::process::id(), SEQ.fetch_add(1, Ordering::Relaxed)));
    std::fs::create_dir_all(p.join("background")).expect("create tempdir");
    p
}

/// Dim is a black overlay at `dim`, applied to the pixels by whoever owns them - and it is exactly
/// the formula `stageBg.ts` uses for the preview's own video draw, so the two agree.
#[test]
fn dim_multiplies_every_channel_and_leaves_alpha_alone() {
    ...
    assert_eq!(none, vec![100, 200, 255, 255], "dim 0 must not touch a single byte");
    let mut full = vec![100u8, 200, 255, 255];
    apply_dim(&mut full, 1.0);
    assert_eq!(full, vec![0, 0, 0, 255], "alpha is never dimmed - this is an overlay, not a fade");
}

/// The dim the EXPORT applies is the one the settings publish, clamped - not the raw field.
#[test]
fn build_applies_the_clamped_dim_to_a_solid() {
    let s = BackgroundSettings { kind: BackgroundKind::Solid, solid: [200, 100, 50], dim: 9.0, ..Default::default() };
    let buf = build(&s, &[], 2, 2, Path::new("."));
    // BGRA, and dim_clamped(9.0) == 0.8 -> every channel * 0.2.
...
fn a_missing_asset_falls_back_to_the_base_wallpaper_instead_of_failing() {
    let dir = tempdir();
    let s = BackgroundSettings { kind: BackgroundKind::Image, asset: Some("background/gone.png".into()),
        solid: [1, 2, 3], ..Default::default() };
    let got = build(&s, &[], 4, 4, &dir);
    assert_eq!(got.len(), 4 * 4 * 4, "a missing asset must still produce a full background");
    assert!(video_source(&s, &dir).is_none(), "a missing file is never a decode source");
...
fn video_source_is_only_the_video_kind_with_a_real_file() {
    ...
    let v = BackgroundSettings { kind: BackgroundKind::Video, asset: Some("background/loop.mp4".into()), ..Default::default() };
    assert_eq!(video_source(&v, &dir), Some(dir.join("background").join("loop.mp4")));
    let i = BackgroundSettings { kind: BackgroundKind::Image, ..v.clone() };
    assert!(video_source(&i, &dir).is_none(), "an image never opens a decode stream");
    let m = BackgroundSettings { kind: BackgroundKind::Mesh, ..v.clone() };
    assert!(video_source(&m, &dir).is_none(), "an asset kept while a wallpaper is showing stays idle");
    let _ = std::fs::remove_dir_all(&dir);
}
