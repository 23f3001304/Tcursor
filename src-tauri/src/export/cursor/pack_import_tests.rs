// Split from pack_import.rs per repo convention (#[path] sibling test module).
// NOTE: `import_cursor_pack`'s success path writes under the REAL `cursors_dir()` (app-data),
// so only its early (pre-write) error paths are exercised directly here; the write/assembly
// logic itself is fully covered via the pure `collect_valid_sprites`/`read_and_validate_hotspots`/
// `write_pack` helpers against temp folders (mirrors settings::store's own test, which checks
// `config_path()`'s shape but never calls the real `save()`).
use super::*;
...
fn temp_dir(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join("tcursor_pack_import_test").join(name);
...
fn collect_valid_sprites_only_keeps_recognized_names_with_real_png_bytes() {
    ...
    std::fs::write(dir.join("arrow.png"), TINY_PNG).unwrap();          // recognized + valid
    std::fs::write(dir.join("hand.png"), b"not a png").unwrap();       // recognized, invalid bytes
    std::fs::write(dir.join("unknown_name.png"), TINY_PNG).unwrap();   // valid bytes, unrecognized name
...
fn write_pack_creates_pngs_hotspots_and_meta() {
    // A nested, not-yet-existing folder - only its parent is pre-created - so this also
    // proves write_pack creates the pack dir itself (create_dir_all), not just files in it.
    let dir = temp_dir("write_pack_out").join("pack_dir");
    let sprites = vec![("arrow.png".to_string(), TINY_PNG.to_vec())];
    write_pack(&dir, "my_id", "My Name", &sprites, Some(br#"{"arrow":[0.2,0.3]}"#)).unwrap();
    ...
    let meta: serde_json::Value = serde_json::from_slice(&std::fs::read(dir.join("pack.json")).unwrap()).unwrap();
...
fn write_pack_defaults_hotspots_to_empty_object_when_none_given() {
    ...
    assert_eq!(std::fs::read_to_string(dir.join("hotspots.json")).unwrap(), "{}");
}
