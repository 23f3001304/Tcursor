...
describe("truncateToastText", () => {
  ...
  it("truncates a long raw warning on a word boundary with an ellipsis", () => {
    const text = "webcam decode failed after 143 frames - the camera panel is frozen from that point on: decoder returned an unexpected end of stream while reading packet 88213";
    const out = truncateToastText(text);
    expect(out.length).toBeLessThanOrEqual(120);
    expect(out.endsWith("…")).toBe(true);
    expect(out.endsWith(" …")).toBe(false); // no trailing space before the ellipsis
  });
