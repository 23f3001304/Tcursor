# src-tauri/src/export/fx/lens/mask.rs

The lens **mask**: which pixels of a frame the glass covers, and how one cursor state's silhouette dissolves into the next. `lens/mod.rs` owns the shapes and the curves; the silhouette work is its own responsibility and lives here. Both halves are `pub use`d from there, so callers keep saying `lens::mask_of`.
...
The sprite's own opacity is not lost: it still shows, in the 65%-alpha blit that lands on top (`lens::SPRITE_ALPHA`).
...
- `src-tauri/src/export/fx/lens/build.rs` - `lens_of`, only while `kind_morph` reports a change still in flight.
