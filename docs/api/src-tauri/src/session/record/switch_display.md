...
```rust
#[tauri::command]
pub fn switch_display(target_id: String, recorder: State<'_, Recorder>, platform: State<'_, Arc<Platform>>, app: AppHandle) -> Result<(), String>
```
...
2. Build a `ports::capture::CaptureRequest` from the take's own `clock` / `stop` / `paused` / `paused_totals`, a fresh `record-ended-early` emitter, `fps` from `platform.system.primary_refresh_hz().min(60)`, `prefer_compatibility: false` and `with_cursor: false`. *Why fps is recomputed rather than remembered:* the encoder is already built, so this value only sets the new capture's `MinimumUpdateIntervalSettings` floor. *Why `output` is the take's own `video.mp4`:* a switch keeps the encoder, so the file it is writing does not change; the field is the truth about where the frames go, not a placeholder. *Why the cursor is still off:* it is captured as its own layer (`ports::input::CursorShapePort`), which is what keeps every cursor style switchable in the editor.
3. `at_ms` from `recording_ms` (the pause-compressed recording clock, the same one the frame timestamps and the mic segments are on; see "The switch instant" below), then `platform.capture.bounds(&target)` for the target's `CaptureGeometry` - a monitor's rectangle, or a window's VISIBLE frame, which is what WGC captures. *Why the bounds come from the capture port and not a system one:* starting a capture and asking for its rectangle have to resolve the same target the same way; they once did not, and `display:N` took another monitor's origin and shifted every cursor point.
4. Push `DisplaySwitch { at_ms, target_id, w, h }` into `Running.segments.displays` (written into `sync.json` at Stop by `recorder_threads::save_session_files`) BEFORE the restart, and build the `ports::capture::FirstFrameSize` hook that corrects it: a closure over the shared log calling `SegmentLog::set_display_size(at_ms, fw, fh)` with the replacement capture's first frame size, the moment `Cap::on_frame_arrived` sees it. *Why the correction:* `export::render::spans` crops the fitted picture by `letterbox((w, h), canvas)` to the pixel, and a window rect 14 px wider than its capture (the invisible DWM borders, `GetWindowRect`'s size) left a black edge either side of the picture in the owner's first two-monitor take (2026-09-14). *Why push first:* that first frame can arrive before `switch` returns.
5. `Running.video.switch(req, on_size)` - the restart, through `ports::capture::VideoSink`. On `Err` the record is removed again (`retain` on `at_ms`; there is no capture for it to describe) and the error returned. On Windows `FirstFrameSize` and `gpu::frames::SizeHook` are literally the same type, so the hook passes through the port untouched.
6. Compare the rectangle with `Running.screen`, the take's own `ScreenInfo`. If they match, the take is back on the display it started on and the remap is cleared with `None` rather than replaced by an identity `Remap`, so the ordinary case costs the hook nothing. Otherwise install `Remap::for_switch(...)` on `Running.mouse`, through `PointerPort::set_remap`.
