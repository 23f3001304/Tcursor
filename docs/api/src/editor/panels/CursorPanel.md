...
- On failure: shows the rejection message (or a generic fallback if it isn't a string) via the shared `.e-errline` class (also used by `AiPanel`'s Engine-picker error line) below the grid.
