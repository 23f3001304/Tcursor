# src-tauri/src/events/typing.rs
...
- `src-tauri/src/session/recorder_threads.rs` - the keyboard hook appends to a `Vec<u32>` and calls `save` at session end
...
- `src-tauri/src/export/exporter.rs` - the export pipeline loads it for the same purpose
