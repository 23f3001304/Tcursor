...
describe("remapDoc (parity with edit::remap_doc)", () => {
  ...
  it("moves captions and their own word timings onto the output clock", () => {
    const d = base();
    d.captions.push({
      id: "c0",
      start_ms: 2200,
      ...
      text: "hello world",
      words: [
        { start_ms: 2200, end_ms: 2600, text: "hello" },
        { start_ms: 2600, end_ms: 3200, text: "world" },
      ],
    });
    d.captions.push({ id: "c1", start_ms: 1100, end_ms: 1900, text: "gone", words: [] });
    const r = remapDoc(d, fixtureMap());
    expect(r.captions.map((c) => [c.id, c.start_ms, c.end_ms])).toEqual([["c0", 700, 1350]]);
    expect(r.captions[0].words.map((w) => [w.start_ms, w.end_ms])).toEqual([
      [700, 1050],
      [1050, 1350],
    ]);
  });
  ...
  it("rewrites clip_ms to the output duration, as edit::remap_doc does", () => {
    const r = remapDoc(base(), fixtureMap());
    expect(r.clip_ms).toBe(outDurMs(fixtureMap()));
    expect(remapDoc(base(), identityMap(60_000)).clip_ms).toBe(60_000);
  });
