...
// The overlay plus the ONE keyboard path a selected cut is removed through - the shared
// delete-selection branch in `useEditorKeymap`, not a listener of the overlay's own. Mounting both
// in the same harness is what proves the two halves are actually wired to each other.
const DUR = 10_000;
const DOC = {
  cuts: [{ id: "c0", start_ms: 1000, end_ms: 2000 }, { id: "c1", start_ms: 4000, end_ms: 4500 }],
  speed: [], zooms: [], effects: [], layout: [], camera_moves: [],
...
const press = (el: Element | null) => act(() => { el?.dispatchEvent(new MouseEvent("pointerdown", { bubbles: true })); });
const key = (k: string) => act(() => { window.dispatchEvent(new KeyboardEvent("keydown", { key: k })); });

function Harness() {
  const [s, setS] = useState<string | null>(null);
  sel = s;
  const applyOp = async (op: EditOp) => { ops.push(op); return null; };
  useEditorKeymap({
    sel: s, doc: DOC, timeMs: 0, setSel: setS, setPlaying: () => {}, applyOp,
    addZoom: async () => {}, addSpotlight: async () => {}, onOverlay: () => {},
    modalOpen: false, shortcutsOpen: false, escOwned: false,
  });
...
describe("CutOverlay", () => {
  ...
  it("a click selects that cut, which is what opens its inspector", () => {
    ...
    expect(ops).toEqual([]); // selecting is not an edit
  });
  ...
  it("Delete on a selected cut removes it through the shared delete-selection path", () => {
    ...
    expect(sel).toBeNull(); // the selection goes with the thing it pointed at
  });
