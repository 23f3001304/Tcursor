# src/editor/panels/camera/CameraMoveField.tsx
...
**`commit` keeps a pending drag, and keeps it CURRENT.** It fires on every slider tick and reads `camDraftRef.current` each time, folding the draft's x/y into the keyframe so a drag in progress is not lost mid-gesture. It also writes the committed size AND corner fraction back into the draft (`{ ...d, size, round: shapeRound(shape, roundness) ?? undefined }`, with the shape/roundness taken from the patch or else the keyframe at the playhead; `"layout"` leaves `round` undefined = inherit). *Why that matters (the size half fixed 2026-09-14, the shape half in the 2026-09-15 sweep):* the composite draws `camDraftRef` in PREFERENCE to the keyframe (`frameCamLayout`'s `drag ?? camMoveAt(...)`), so a draft left holding the pre-slider size, or no `round` at all, kept the PiP as it was no matter what the control wrote - after any drag, the size slider, then the shape picker, looked dead. Updating the draft in step closes that gap, and also means a PiP drag started right after seeds `start.size` from the size just committed rather than a stale one. The draft is still never CLEARED here - only by the button below, by moving the playhead, or by Move mode ending (`useMoveModeGuard`).
...
- `src/editor/panels/camera/CameraPanel.tsx` - since 2026-09-14, essentially the WHOLE panel: everything else there was per-layout appearance and moved to Layouts (`CameraPanel.md`), leaving the header, this field, and one hint line. `staticSize` still comes from `settings.screen.cam_size`.
