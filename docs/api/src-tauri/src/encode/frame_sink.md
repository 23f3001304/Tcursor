...
- `split(&mut self) -> std::io::Result<()>` - *Called once on each pause -> resume boundary of a live recording (`RecordingSession::run`). Defaulted to a no-op: only a sink whose timestamps are GENERATED by wall clock rather than handed to it needs the boundary, because that is the one design where a paused span leaves a real gap in the output's own PTS. In practice that is `VfrSegments`, which starts a new MP4 part per unpaused span; every other sink (including `FfmpegFrameSink` used directly for export) inherits the default.*
...
- `encode/vfr_segments.rs` - `VfrSegments` implements `FrameSink` over a sequence of `FfmpegFrameSink` parts, and is the only implementor that overrides `split`.
