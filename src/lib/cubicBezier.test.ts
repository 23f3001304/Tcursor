...
import { ease } from "../editor/timeline/layoutTrack";
...
describe("parseCubic", () => {
  ...
  it("rejects garbage, exactly like the Rust parser", () => {
    for (const s of ["smooth", "cubic(", "cubic()", "cubic(1,2,3)", "cubic(1,2,3,4,5)",
      "cubic(a,b,c,d)", "cubic(0.1,0.2,0.3,)", "bezier(0,0,1,1)", "", null, undefined]) {
...
describe("evalCubic (parity with Rust export::cubic::eval)", () => {
  it("has exact endpoints and clamps its input", () => {
    for (const c of [[0.25, 0.1, 0.25, 1], [0, 0, 1, 1], [0.34, 1.56, 0.64, 1]] as const) {
  ...
  it("is monotonic in x for in-range handles", () => {
    for (const [x1, x2] of [[0, 1], [1, 0], [0, 0], [1, 1], [0.25, 0.25]]) {
      let prev = -1;
      for (let i = 0; i <= 100; i++) {
...
describe("ease() dispatches custom cubics", () => {
  ...
  it("leaves the six named curves untouched and still defaults garbage to smooth", () => {
    ...
    expect(ease("cubic(1,2)", 0.5)).toBeCloseTo(0.5, 6); // malformed -> smoothstep, which is 0.5 at 0.5
