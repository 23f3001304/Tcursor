...
fn a_sprite_box_puts_the_hotspot_on_the_cursor_point() {
    ...
    assert_eq!(
        b,
        [90.0, 95.0, 20.0, 10.0],
        "hotspot at the centre of a 20x10 box"
    );
...
fn the_box_interpolates_from_the_previous_kind_to_the_new_one() {
    ...
    assert_eq!(
        start,
        sprite_box(&disc, (100.0, 100.0), 40.0, 1.0),
        "m=0 is the previous box"
    );
    assert_eq!(
        end,
        sprite_box(&pill, (100.0, 100.0), 40.0, 1.0),
        "m=1 is the new one"
    );
    let mid = at(0.5);
    for i in 0..4 {
        assert!(
            (mid[i] - (start[i] + end[i]) * 0.5).abs() < 1e-4,
            "component {i} is the midpoint"
        );
    }
    for m in [0.0, 0.25, 0.5, 0.75, 1.0] {
        let b = at(m);
        assert!(
            (b[0] + b[2] * 0.5 - 100.0).abs() < 1e-4 && (b[1] + b[3] * 0.5 - 100.0).abs() < 1e-4
        );
    }
}
...
fn a_settled_morph_takes_the_new_box_without_touching_the_old_sprite() {
    ...
    assert_eq!(
        morph_box(&a, &b, 1.0, (0.0, 0.0), 20.0, 1.0),
        sprite_box(&b, (0.0, 0.0), 20.0, 1.0)
    );
    assert_eq!(
        morph_box(&a, &b, 1.5, (0.0, 0.0), 20.0, 1.0),
        sprite_box(&b, (0.0, 0.0), 20.0, 1.0)
    );
}
