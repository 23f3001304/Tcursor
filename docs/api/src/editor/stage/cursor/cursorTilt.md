# src/editor/stage/cursor/cursorTilt.ts

The live preview's mirror of Rust `export/cursor/draw/tilt.rs` - the **motion tilt**: the very slight lean a cursor takes on when it is thrown across the screen, and the single overshoot it corrects through when it stops.
