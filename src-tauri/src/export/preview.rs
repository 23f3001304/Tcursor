...
/// PNG-encode a BGRA buffer via ffmpeg. Writes bgra to stdin on a thread to
/// avoid deadlock, reads PNG bytes from stdout, returns them.
pub(crate) fn png_encode(bgra: &[u8], w: u32, h: u32) -> Result<Vec<u8>> {
    let mut child = ffcmd("ffmpeg")
        ...
        .spawn()
        .context("spawn ffmpeg png encoder")?;

    let mut stdin = child.stdin.take().context("png encoder stdin missing")?;
    let bgra_owned = bgra.to_vec();
    let writer = std::thread::spawn(move || {
        let _ = stdin.write_all(&bgra_owned);
    });

    let out = child.wait_with_output().context("png encoder wait")?;
    let _ = writer.join();
    if out.stdout.is_empty() {
        anyhow::bail!("png encoder produced no output: {}", String::from_utf8_lossy(&out.stderr));
    }
    Ok(out.stdout)
}
