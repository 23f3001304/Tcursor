...
6. Call `resolveSpotlight` **exactly once** per tick (mutates the persistent `spotSimRef` in place - a second call at the same time would double-advance the same stateful transition). Compute a cache key (`fxCacheKey`, `fxCacheKey.ts`) from that one resolved spotlight, the zoom-projected cursor position, click track, and click-FX params, bucketed to `FX_BUCKET_MS` (~25fps, `timeBucket`) - **not** every rAF tick - and record it in `fxWantRef` unconditionally (whether or not a request actually fires this tick). If the key differs from `fxLastTRef` (the last APPLIED key) and no request is already in flight, call `requestFxOverlay` at `FX_SCALE` (0.5x) resolution, passing that SAME resolved spotlight straight through (`requestFxOverlay` no longer resolves it itself).
7. In the response handler: clear `fxInflightRef` first (always, success or failure, so a rejected request can't wedge the loop). Feed the requested key, `fxWantRef.current`, and the resolved `url` to `fxResponseAction` (`fxCacheKey.ts`). A `"stale"` action means the desired key moved on while the request was outstanding, so it's dropped outright - applying it would blit an outdated spotlight/click frame, and leaving it unlatched means the very next tick reissues a request for whatever is actually wanted now. An `"apply"` action - covering BOTH a real image AND a `null` `imageUrl` (nothing to draw at this key: fx disabled, or no active click/spotlight, the common case) - latches `fxLastTRef` to the requested key, since both are equally a landed, valid terminal answer for that key; only an actual REJECTED request (caught separately, below) skips the latch. A non-null `imageUrl` decodes into `fxOverlayImgRef`; a `null` one explicitly clears it (`fxOverlayImgRef.current = null`) so a stale image never lingers after the spotlight/clicks end. Before this, a `null` response fell into neither latch path, so the paused, no-fx-active preview (the common case) re-requested the overlay every tick forever.
