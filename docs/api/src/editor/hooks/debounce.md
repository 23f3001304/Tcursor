...
- `Slider` (`src/editor/controls/Slider.tsx`) - wraps the caller's `onChange` so a pointer drag
...
- `useEditorData` (`src/editor/hooks/useEditorData.ts`) - debounces the `preview_bg` refetch
