# src-tauri/src/win/sys/display.rs
...
- `src-tauri/src/session/record/recorder.rs` - calls `.min(60)` on the result to cap WGC capture at 60 fps (the maximum TCursor encodes at), preventing unnecessary CPU load on 120/144 Hz displays
- `src-tauri/src/export/pipeline/run.rs` - reads the primary refresh rate (capped at 60) to set the encode frame rate for exported video
