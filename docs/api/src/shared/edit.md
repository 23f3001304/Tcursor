...
- `src/shared/edit.ts` - field `Zoom.target`.
- `src/shared/ipc.ts` - appears in `EditOp` variant `update_zoom`'s `target?` field.
...
- `layer: number` - stacking index among zooms. Assigned by `layoutRegions` (`src/editor/timeline/model/layers.ts`) for timeline row placement, and read by `resolveCamAction` (`src/editor/stage/camera/camZoomAction.ts`) to pick which of several time-overlapping zooms' `cam_action` governs the webcam - the highest layer wins, ties going to the last one, mirroring the Rust `max_by_key`.
...
- `easing_out?: string | null` - the ZOOM-OUT ramp's curve (M3). Absent or `null` means "the same curve as `easing`", which is what every doc written before this field existed carries and exactly today's behaviour. Rust omits the key entirely (`skip_serializing_if`) whenever the two ramps agree, so a project that never splits them writes the JSON it always did; `presetOf` (`src/editor/motion/presets.ts`) reads a missing value as "same as `easing`" for the same reason. Written through `update_zoom`'s `easing_out`.

### Used by

- `src/shared/edit.ts` - field `EditDoc.zooms`.
- `src/shared/ipc.ts` - `getEdit` and `applyEditOp` return `EditDoc` which contains `zooms: Zoom[]`.
...
- `src/editor/stage/camera/camZoomAction.ts` - `resolveCamAction` reads `layer` and `cam_action` to resolve the webcam's behavior during overlapping zooms.
...
- `src/shared/edit.ts` - field `EditDoc.cuts`.
- `src/shared/ipc.ts` - present in `EditDoc` returned by `getEdit` and `applyEditOp`.
...
- `src/shared/edit.ts` - field `EditDoc.speed`.
- `src/shared/ipc.ts` - present in `EditDoc`.
...
- `src/shared/edit.ts` - fields of `Arrangement`; the `set_arrangement` `EditOp` payload.
- `src/shared/ipc.ts` - inside `LayoutPresetDto.arrangement`.
...
- `src/shared/edit.ts` - the optional `LayoutSeg.arrangement` field.
- `src/shared/ipc.ts` - `LayoutPresetDto.arrangement`, the pose form of each preset.
...
- `src/shared/edit.ts` - field `EditDoc.layout`.
- `src/shared/ipc.ts` - present in `EditDoc`; the `add_layout_seg` / `update_layout_seg` / `remove_layout_seg` `EditOp` variants add, patch, and delete segments by id.
...
- `src/editor/timeline/model/layoutTrack.ts` - `layoutAt` cross-fades between segments' resolved panel rects for the preview, honoring each segment's own `transition_ms`/`easing`.
...
- `src/shared/edit.ts` - field `EditDoc.trim`.
- `src/shared/ipc.ts` - `set_trim` `EditOp` variant sets both fields atomically.
...
- `src/shared/edit.ts` - field `EditDoc.aspect`; `EditOp` variant `set_aspect`.
- `src/editor/stage/transport/Transport.tsx` - the aspect chip cycles through all 5 values and calls `set_aspect`.
...
- `src/editor/timeline/lanes/TrimOverlay.tsx` - the dimmed head/tail regions and drag-handle positions.
- `src/editor/hooks/doc/useTrimActions.ts` - resolves the current range once so `onTrimIn`/`onTrimOut` can reject a playhead click that would collapse or invert the trim.
...
- `src/shared/ipc.ts` - return type of `getEdit`, `applyEditOp`; parameter type of `saveEdit`.
...
```ts
export interface CaptionWord { start_ms: number; end_ms: number; text: string }
```

One spoken word with its own timing, from the ASR pass - mirrors Rust `edit::captions::CaptionWord`. Times are OUTPUT-clock ms, the same clock every other lane uses.

### Used by

- `src/shared/edit.ts` - field `Caption.words`.

## Caption

```ts
export interface Caption { id: string; start_ms: number; end_ms: number; text: string; words: CaptionWord[] }
```

One caption line on the timeline - mirrors Rust `edit::captions::Caption`. `words` is empty on a caption typed by hand or split without word timings; the word-by-word highlight simply turns off for it.

### Used by

- `src/shared/edit.ts` - field `EditDoc.captions`.
...
- `src/shared/edit.ts` - field `EditDoc.camera_moves`.
- `src/editor/stage/camera/cameraMoves.ts` - samples/interpolates between keyframes (`CamPose`) for the live preview.
...
- `src/editor/timeline/lanes/CameraLane.tsx` - renders and drags keyframes on the timeline.
