...
import { graded } from "./gradeFixture";
import { COLOURS, PRESETS } from "./gradeFixture";

function near(got: number[], want: [number, number, number], where: string) {
  for (let k = 0; k < 3; k++) {
    expect(Math.abs(got[k] - want[k]), `${where} ch ${k}`).toBeLessThanOrEqual(1);
  }
}
...
describe("the TypeScript grade is the Rust grade", () => {
  it("matches at the frame centre for every preset and colour, within one byte", () => {
    let i = 0;
    for (const p of PRESETS) {
      for (const c of COLOURS) {
        near(graded(p, c, 0, 0), CENTRE[i], `${p} ${c} centre row ${i}`);
        i++;
      }
    }
    expect(i).toBe(108);
  ...
  it("matches at an edge midpoint and a corner for the two heaviest vignettes", () => {
    let i = 0;
    for (const p of ["midnight", "noir"] as GradePreset[]) {
      for (const [u, v] of [
        [0, 0],
        [0.5, 0],
        [0.5, 0.5],
        ...
        for (const c of [
          ...
          near(graded(p, c, u, v), POSITIONS[i], `${p} ${c} (${u},${v}) row ${i}`);
          i++;
        }
      }
    }
