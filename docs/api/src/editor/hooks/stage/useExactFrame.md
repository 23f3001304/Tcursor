...
**The instant is OUTPUT time since Batch 4 (clips).** A clip list can reorder the recording, so one source instant can be shown twice and clip time no longer names a single showing; the backend's `out_of` answers with the first one, which is the wrong frame whenever the playhead rests in the second. The hook takes `outMs` and keys on it, `useStageEngine` passes the `tOut` it already computes, and `drawCompositeFrame` compares the held frame's key on the same clock. Inside a segment output and clip time round trip exactly, so an unsplit project's paused frame does not move.
...
`"<rounded output instant>|<edit generation>"`. A frame from another instant or an older doc never matches, so it is never drawn stale. The shape did not change when the clock did; what changed is that both ends now feed it output ms, the hook when it files a fetched frame and `drawCompositeFrame` when it asks whether the held one is this tick's. Tests: `useExactFrame.test.ts`.
...
```ts
export function useExactFrame({ folder, playing, outMs, draft, dirtyRef, deps }: {
  folder: string; playing: boolean; outMs: number; draft: boolean; dirtyRef: RefObject<boolean>; deps: unknown[];
...
- `playing` / `outMs` - the transport. The request keys on `outMs` in OUTPUT time, the same clock `preview_frame` takes.
...
- `deps` - everything that changes what a frame looks like except the playhead (`Stage` passes its draw deps minus the transport, plus `moveMode` and `bg`). Any change bumps the edit generation, which retires the held frame (its key no longer matches) and, after `SETTLE_MS`, requests a fresh one.

### Returns

`exactRef` (the held frame, or null) and `editGenRef` (the current generation), both for `useCompositeLoop`, which draws `exactRef.current.img` over the composite on a paused tick whose `exactKey(tOut, editGenRef.current)` equals the held key.

### Behavior

- One effect on `[folder, playing, outMs, draft, editGen]`: when `wantsExact`, start a `SETTLE_MS` timer, then `previewFrame(folder, outMs)`, decode the data URL into an `Image`, and on load store `{ key, img }` and mark the canvas dirty. The effect's cleanup sets a `live` latch false and clears the timer, so a newer instant, a newer doc, a play or a draft supersedes an in-flight request and its answer is dropped.
...
- `src/editor/stage/useStageEngine.ts` - `useExactFrame({ folder, playing, outMs: tOut, draft: moveMode || arranging, dirtyRef, deps: [...] })`, its refs handed to `useCompositeLoop`.
