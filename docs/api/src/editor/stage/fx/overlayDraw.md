...
Everything drawn ON TOP of the composited base frame, in order: the mirrored click ripples, the spotlight resolve and its cached FX-overlay blit, the animated text items, the captions, then the FX-overlay request/response bookkeeping that primes the NEXT tick's blit. Split out of `compositeFrame.ts`'s `drawCompositeFrame` (editor-parity foundations, task 9) so it is the one small file Batch 2's mask/grade/text agents each add one call to, rather than three agents converging on the same tick function. `drawCompositeFrame` calls it once per tick, right after `drawPreview` paints the background/screen/webcam/cursor and before the paused tick's held exact frame.

The seams Batch 2 uses, mirroring the export's ruled order (`render/fx_step.md`): Batch 2a's mask call goes BEFORE the cached FX-overlay blit, since masks precede the FX pass in `fx_pass`, and it must land UNDER the cursor to match the export - so it goes in the seam Task 0a opened between `drawPreview` and `drawCursorLayer`, not here. Batch 2b's grade covers the composited PICTURE only, never the overlays drawn here, and goes in the same seam. Batch 2c's text goes here, after the FX blit and before `drawCaptions`. The export is the reference: where the two cannot be made to agree by ordering alone, the preview is what changes.

**One deviation stays, and it is deliberate.** This whole file runs after `drawCursorLayer`, so in the preview the ripples, the FX overlay blit, the TEXT and the captions all land ON TOP of the cursor, where the export draws the cursor last of all and therefore over every one of them. Task 0a fixed the half it had to (the cursor moved out of `drawPreview` so the grade could cover the webcam panel without touching the cursor); fixing this half means splitting the cached backend FX overlay into an under-cursor and an over-cursor image, which is a change to `export/preview/preview_fx.rs` and to the FX request protocol, and it belongs to whatever batch takes the preview FX blit apart. The paused exact frame (`useExactFrame`) already shows the true stacking, and has been the standing visual gate since 2026-09-14.
...
4. Draw the animated text items: `drawTexts(ctx, laidTexts(textsRef.current, accentRef.current, c.width, c.height, tOut))` - after the FX blit and before the captions, which is the order `fx_pass` paints them in. `tOut`, like the captions below, because `remapDoc` has already moved the text items onto the output clock. See `../text/textPreview.md` and `../text/textDraw.md`.
5. Draw the spoken captions: `drawCaptions(ctx, c.width, c.height, captionsRef.current, capStyleRef.current, accentRef.current, tOut)` - after the FX blit and after the text, the order `composite_at` paints them in, and the reason for it is spec 4 fit point 1: captions are last and nothing may cover them. Note `tOut`, not `t`: the caption track is a doc region list and `remapDoc` has already moved it (and each caption's own word timings) onto the output clock. See `./captionDraw.md`.
