...
Doc-version migrations plus the event/output clock shift they need; they live in their own file because migrations keep accumulating. `edit::seed::load_or_seed_locked` calls `migrate` right after load/build, before `effects::lift_always_on_spotlight` - see `docs/api/src-tauri/src/edit/seed.md`. `true_duration_ms` is re-exported as `seed::true_duration_ms` so external callers use the `seed::` path.
...
**`shift`/`true_dur` are PRECOMPUTED by the caller, not derived here from `paths` (bug-sweep-2 Task 7 round 2 - H3).** Before this change `migrate` took `paths: &ProjectPaths` and called `output_shift`/`true_duration_ms` (both `build_timeline`/ffprobe-backed) itself. `seed::load_or_seed`'s caller (`edit::seed::derive_seed_inputs`) now precomputes both values BEFORE taking `edit::lock::doc_lock`, so this function's own body is pure in-memory work - keeping the ffprobe-backed cost off the locked critical path entirely. See `docs/api/src-tauri/src/edit/seed.md` for the full design and the monotonicity argument for why a value the caller decided not to precompute can never turn out to have been needed.
...
`shift` is `0` (a no-op) when the timeline couldn't be rebuilt at precompute time - the shift is unknowable, so the regions are left exactly where they are - but the doc is still stamped v2, because re-running the migration later (when a timeline WOULD resolve) would shift regions the user has since edited in output time. `output_shift(paths)` (below) is the function that derives this value; it is re-exported as `seed::output_shift` and `ai::run::propose` reuses this SAME recipe to put the AI director transcript (`ai::plan::transcript::serialize`) on the output clock, rather than re-deriving the shift.

### `output_shift` used by

- `src-tauri/src/edit/seed.rs` - `derive_seed_inputs`, precomputed unlocked and passed into `migrate` as `shift` (not called from `v1_to_v2`/`migrate` directly since bug-sweep-2 Task 7 round 2 - see `seed.md`)
- `src-tauri/src/ai/run.rs` - `propose`, via the `seed::output_shift` re-export
...
- `src-tauri/src/edit/seed.rs` - `derive_seed_inputs`, precomputed unlocked and passed into `migrate` as `true_dur` for its `clip_ms` backfill (bug-sweep-2 Task 7 round 2)
- `src-tauri/src/ai/run.rs` - `propose`'s clip-length fallback
