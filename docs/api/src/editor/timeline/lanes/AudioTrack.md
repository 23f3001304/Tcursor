# src/editor/timeline/lanes/AudioTrack.tsx
...
- `!src && loading` - renders `<Shimmer className="e-audiorow" />` (`src/editor/timeline/lanes/Shimmer.tsx`), the same box (height/radius/background) as the loaded row.
