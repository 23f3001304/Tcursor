# src-tauri/src/platform/windows/input/bitmap.rs

The one Win32 seam of the cursor layer: turn a live `HCURSOR` into the actual bitmap Windows is drawing it from. Windows-only by its place in the tree - `platform/mod.rs` gates `pub mod windows;` - so it needs no `#[cfg]` of its own any more.
...
- `src-tauri/src/platform/windows/input/cursor.rs` - calls it once per distinct `HCURSOR` seen during a take
