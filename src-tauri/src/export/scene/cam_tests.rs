...
fn rect_from_center_is_squared_and_centered() {
    let p = CamPose { x: 0.5, y: 0.5, size: 0.3, round: None };
...
fn rect_from_center_keeps_a_wide_panel_wide() {
    ...
    let r = rect_from_center(CamPose { x: 0.5, y: 0.5, size: 0.25, round: None }, ow, oh, a);
...
fn static_cam_pose_round_trips_a_wide_rect() {
    ...
    let panel = Panel { rect, radius: 108.0, alpha: 1.0, ring_px: 0.0, ring_color: [0, 0, 0] }; // a stadium: radius = short side / 2
    ...
    assert_eq!(p.round, Some(0.5), "the live pose carries the panel's shape as a fraction of its short side");
}
...
fn override_camera_keeps_circle_round_after_resize() {
    ...
    let p = CamPose { x: 0.5, y: 0.5, size: 0.1, round: None }; // shrinks to 10% of oh=1000 -> 100x100
...
fn override_camera_grows_radius_when_panel_grows() {
    ...
    let p = CamPose { x: 0.5, y: 0.5, size: 0.4, round: None }; // grows to 40% of oh=1000 -> 400x400 (4x)
    let out = override_camera(panel, p, 1000.0, 1000.0, 1.0);
...
fn override_camera_matches_rect_from_center_position() {
    ...
    let p = CamPose { x: 0.3, y: 0.7, size: 0.2, round: None };
    let (ow, oh) = (1920.0, 1080.0);
...
// A pose that carries a shape sets the radius from the NEW short side - a keyframed circle is a
// circle at any size, a keyframed rect has no corners at all - while a shapeless pose keeps
// scaling the static radius exactly as before.
#[test]
fn override_camera_takes_the_poses_own_shape_when_it_has_one() {
    let panel = Panel { rect: RectF { x: 0.0, y: 0.0, w: 100.0, h: 100.0 }, radius: 12.0, alpha: 1.0, ring_px: 4.0, ring_color: [1, 2, 3] };
    let circle = override_camera(panel, CamPose { x: 0.5, y: 0.5, size: 0.4, round: Some(0.5) }, 1000.0, 1000.0, 1.0);
    assert!((circle.radius - 200.0).abs() < 1e-3, "half the 400 px short side: {}", circle.radius);
    let rect = override_camera(panel, CamPose { x: 0.5, y: 0.5, size: 0.4, round: Some(0.0) }, 1000.0, 1000.0, 1.0);
    assert_eq!(rect.radius, 0.0);
    let wide = override_camera(panel, CamPose { x: 0.5, y: 0.5, size: 0.2, round: Some(0.25) }, 1000.0, 1000.0, 16.0 / 9.0);
    assert!((wide.radius - 50.0).abs() < 1e-3, "a quarter of the SHORT side (h=200), not the width: {}", wide.radius);
    assert!((wide.ring_px - 8.0).abs() < 1e-3, "the ring still scales by the height ratio");
    let inherit = override_camera(panel, CamPose { x: 0.5, y: 0.5, size: 0.4, round: None }, 1000.0, 1000.0, 1.0);
    assert!((inherit.radius - 48.0).abs() < 1e-3, "no shape: the static 12 px scaled 4x");
}
