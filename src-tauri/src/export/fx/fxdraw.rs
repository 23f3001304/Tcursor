...
impl FxRenderer for CpuFx {
    fn apply(&self, out: &mut [u8], ow: u32, oh: u32, state: &FxState) {
        ...
        // Last, matching `fx_lens.wgsl`'s place at the end of `fs_main`: the glass sits over the
        // spotlight and the click styles, under the cursor sprite the next pass blits.
        if let Some(l) = &state.lens {
            crate::export::fx::fx_lensdraw::draw_lens(out, ow, oh, l, state.color);
        }
    }
}
...
mod tests {
    ...
    fn ripple_paints_a_colored_ring() {
        ...
        let st = FxState { style: ClickFxStyle::Ripple, color: [255, 0, 0], intensity: 1.0,
            hits: vec![FxHit { x: 50.0, y: 50.0, progress: 0.5 }], spot: None, video: None, lens: None };
    ...
    fn glow_brightens_near_click() {
        ...
        let st = FxState { style: ClickFxStyle::Glow, color: [255,255,255], intensity: 1.0,
            hits: vec![FxHit{x:50.0,y:50.0,progress:0.2}], spot: None, video: None, lens: None };
        CpuFx.apply(&mut out, w, h, &st);
    ...
    fn neon_paints_a_bright_ring() {
        ...
        let st = FxState { style: ClickFxStyle::Neon, color: [0,128,255], intensity: 1.0,
            hits: vec![FxHit{x:60.0,y:60.0,progress:0.5}], spot: None, video: None, lens: None };
        CpuFx.apply(&mut out, w, h, &st);
    ...
    fn shockwave_paints_an_expanding_ring() {
        ...
        let st = FxState { style: ClickFxStyle::Shockwave, color: [255,255,255], intensity: 1.0,
            hits: vec![FxHit{x:70.0,y:70.0,progress:0.5}], spot: None, video: None, lens: None };
        CpuFx.apply(&mut out, w, h, &st);
    ...
    fn pulse_paints_a_disc_with_a_white_core() {
        ...
        let mut out = vec![0u8; (w * h * 4) as usize];
        let st = FxState { style: ClickFxStyle::Pulse, color: [0, 0, 255], intensity: 1.0,
            hits: vec![FxHit { x: 100.0, y: 100.0, progress: 0.2 }], spot: None, video: None, lens: None };
        CpuFx.apply(&mut out, w, h, &st);
        let c = ((100 * w + 100) * 4) as usize;
        assert!(out[c] > 200 && out[c + 2] > 40, "white core over the blue disc at the click");
        // Out on the disc's rim (r = h*(0.01+0.025*ease(0.2)) ~ 4.2px) it is the tint, not white.
        let rim = ((100 * w + 104) * 4) as usize;
        assert!(out[rim] > out[rim + 2], "the rim is the tint (B at idx 0), not the white core");
    }
    #[test]
    fn every_style_flashes_white_at_the_instant_of_the_click() {
        // Design item 2, CPU side: the impact flash is style-independent, so a BLACK tint - which
        // contributes nothing of its own on an ADDITIVE style - still lights the click point.
        // Ripple and Pulse are not probed this way on purpose: they composite their tint OVER the
        // flash (`blend`, not `add_blend`), so a black one would paint the flash back out at the
        // exact centre. Their own tests cover them, and `fx_gpu_tests.rs` pins the shared curve.
        for style in [ClickFxStyle::Glow, ClickFxStyle::Shockwave,
                      ClickFxStyle::Particles, ClickFxStyle::Neon] {
            let (w, h) = (120u32, 120u32);
            let mut out = vec![0u8; (w * h * 4) as usize];
            let st = FxState { style, color: [0, 0, 0], intensity: 1.0,
                hits: vec![FxHit { x: 60.0, y: 60.0, progress: 0.0 }], spot: None, video: None, lens: None };
            CpuFx.apply(&mut out, w, h, &st);
            let c = ((60 * w + 60) * 4) as usize;
            assert!(out[c] > 200, "{style:?}: flash at the click (got {})", out[c]);
        }
    }
    #[test]
    fn particles_paint_multiple_specks() {
        ...
        let st = FxState { style: ClickFxStyle::Particles, color: [255,255,255], intensity: 1.0,
            hits: vec![FxHit{x:80.0,y:80.0,progress:0.5}], spot: None, video: None, lens: None };
        CpuFx.apply(&mut out, w, h, &st);
    ...
    fn empty_state_leaves_frame_untouched() {
        ...
        let st = FxState { style: ClickFxStyle::Ripple, color: [255,0,0], intensity: 1.0, hits: vec![], spot: None, video: None, lens: None };
        CpuFx.apply(&mut out, w, h, &st);
