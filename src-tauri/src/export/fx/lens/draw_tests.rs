use super::*;
use crate::export::fx::lens::LensMask;
use std::sync::Arc;
...
fn the_back_tints_inside_and_shadows_below() {
    ...
    draw_lens(
        ...
        &Lenses {
            glass: None,
            back: Some(disc(40.0, 40.0, 12.0)),
        },
        [255, 0, 0],
    );
    assert!(
        at(&out, w, 40, 40)[0] > 100,
        "the disc lightens what is under it"
    );
    assert!(at(&out, w, 40, 53)[0] < 100, "a drop shadow under the rim");
    assert_eq!(
        at(&out, w, 4, 4),
        [100, 100, 100],
        "nothing outside the shape is touched"
    );
}
...
fn a_live_ink_drop_paints_the_accent_inside_the_back() {
    ...
    assert!(
        p[0] > p[2],
        "blue accent (B at BGRA idx 0) over the neutral tint, got {p:?}"
    );
}
...
fn the_glass_lens_only_touches_pixels_the_sprite_alpha_covers() {
    ...
    assert_eq!(
        at(&out, w, 35, 30),
        [100, 100, 100],
        "the transparent half is left alone"
    );
}
...
fn the_back_magnifies_what_is_under_it_about_its_centre() {
    ...
    assert_eq!(
        before,
        [100, 100, 100],
        "x=42 is background before the lens"
    );
    draw_lens(
        &mut out,
        w,
        ...
        &Lenses {
            glass: None,
            back: Some(disc(40.0, 40.0, 20.0)),
        },
        [0, 0, 0],
    );
    assert!(
        at(&out, w, 42, 40)[1] > 200,
        "inside the disc the stripe is ~1.35x wider: {:?}",
        at(&out, w, 42, 40)
    );
    assert!(
        at(&out, w, 50, 40)[1] < 130,
        "but not everywhere: {:?}",
        at(&out, w, 50, 40)
    );
    assert_eq!(
