...
Rasterizes `text` into the bottom band of the frame, centered horizontally, using Inter SemiBold with a 1px drop shadow.
...
`()`. No-op when `text` is empty or `alpha <= 0.0`. Returns silently if the embedded font cannot be parsed.

### Implementation

1. Guard: return immediately if `text.is_empty() || alpha <= 0.0`.
2. Load the embedded Inter SemiBold font from `FONT` bytes via `ab_glyph::FontRef`.
...
4. Sum horizontal advances of all characters to get the total text width; compute `x` start for centring.
5. For each character, position a `Glyph` at `(x, baseline)` where `baseline = oh - oh * 0.06`. Call `font.outline_glyph(g).draw(...)` to rasterize coverage samples.
6. For each coverage sample at `(gx, gy)`: call `put` twice - first at `(bx + 1, by + 1)` with black `[0,0,0]` at `cov * alpha * 0.6` (shadow), then at `(bx, by)` with white `[255,255,255]` at `cov * alpha` (text). *Why 0.6 shadow alpha:* strong enough to separate the glyph from any background colour while not producing a harsh dark blob.
7. Advance `x` by `scaled.h_advance(gid)`.
