# src-tauri/src/shell/brand_icon.rs

Dynamic app icon + Windows taskbar progress (Task 39 - "the wave lives"): swaps the main window's icon between the normal brand mark and a REC-lit variant while recording, and drives the taskbar progress bar during export. Both are pure brand flair - graceful no-ops on any failure, so a window-manager quirk or a headless run can never turn a decorative touch into a crash or a stuck recording/export. It lived in `win/sys/` with `#[cfg(windows)]` gates until cross-platform Phase 1, Batch B (2026-09-15); the gates went because nothing here is a Windows API call - Tauri implements `set_icon` and `set_progress_bar` on every desktop platform, and a platform that lacks one returns `Err`, which is ignored like every other failure. The one thing still worth knowing for Studio: both functions look up the window named `"main"`, so a second window does not get the REC icon or the progress bar today.
...
Decodes the matching bytes via `decode_png` and calls `win.set_icon(img)`, both `?`-free (`let Some(..) = .. else { return }` / `let _ =`) - any failure (missing window, undecodable bytes, OS rejection) is silently swallowed.
...
Calls `win.set_progress_bar(progress_state(pct))`, swallowing any failure.
