...
const LIVE: CamPose = CamPose { x: 0.85, y: 0.15, size: 0.18, round: None };
...
fn entry_blend_is_continuous_at_both_ends() {
    let track = two_kf_track();
    let want_kf = CamPose { x: 0.20, y: 0.70, size: 0.10, round: None };
...
fn exit_blend_tracks_a_live_pose_that_is_still_moving() {
    ...
    let live_at = |t: u32| {
        ...
        CamPose { x: 0.2 + 0.6 * u, y: 0.2 + 0.4 * u, size: 0.1 + 0.2 * u, round: None }
    };
...
fn single_keyframe_is_a_bump_not_a_whole_clip_override() {
    ...
    assert_eq!(at_kf, CamPose { x: 0.25, y: 0.75, size: 0.30, round: None });
...
fn without_a_live_pose_the_blends_snap_to_the_nearest_end_keyframe() {
    ...
    assert_eq!(track.sample(1700, None), Some(CamPose { x: 0.20, y: 0.70, size: 0.10, round: None }));
    assert_eq!(track.sample(4300, None), Some(CamPose { x: 0.60, y: 0.30, size: 0.40, round: None }));
}
