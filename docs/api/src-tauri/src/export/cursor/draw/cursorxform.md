# src-tauri/src/export/cursor/draw/cursorxform.rs
...
- `src-tauri/src/export/cursor/draw/cursordraw.rs` - `draw_cursor_posed`, only when `BusyPose::is_identity` is false

### Preview parity

The canvas mirror is `drawPosed` in `src/editor/stage/cursor/cursorPreview.ts`: a `translate`/`rotate`/`scale` about the cursor point, which is the same transform about the same anchor. The preview relies on Canvas2D's own resampling rather than reimplementing the bilinear loop, so the two are geometrically identical but not pixel-identical - which is the existing arrangement for every other part of the preview.
