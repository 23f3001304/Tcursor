...
- `the_mask_and_grade_stages_agree_between_the_gpu_and_the_cpu` - the only test that reads a PIXEL out of `mask_fx` or `grade_fx`. A colour ramp (so a pixelated cell differs from the pixels it replaces and a wrong sample point shows) goes through two states in turn: a Highlight mask at `dim = 0.6` under the Noir grade, and a Pixelate mask at a nine pixel cell under Cinematic. Every channel of every pixel must agree with `CpuFx` within the same ±3/255 the spotlight test uses. *Why it pins the ORDER and not just the two stages:* swapping `mask_fx` and `grade_fx` in `fs_main` leaves the whole suite green otherwise, and it moves the Highlight case 26 of 255 - the dim multiplies a graded pixel instead of the grade taking a dimmed one. *Why the cell is nine and not eight:* an odd cell puts the sample point at a texel centre, where the bilinear sampler returns the texel itself; an even one lands on a texel boundary and the shader averages two of them while `maskdraw` takes one. *Why a Highlight and not a Blur:* the shader's fixed 13-tap disc and the CPU's three box passes are a documented approximation of each other (see `fx_mask.wgsl`), so a Blur cannot be held to a per-pixel tolerance.
