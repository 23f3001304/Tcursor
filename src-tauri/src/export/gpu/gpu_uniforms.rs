...
pub fn build_uniforms(
    ...
    Uniforms {
        ...
        screen_r: scene.screen.radius,
        camera_r: scene.camera.radius,
        screen_a: scene.screen.alpha,
        camera_a: if webcam.is_some() {
            scene.camera.alpha
        } else {
            0.0
        },
        screen_w: scene.screen.rect.w,
        screen_h: scene.screen.rect.h,
        cam_w: scene.camera.rect.w,
        cam_h: scene.camera.rect.h,
        wc_aspect: webcam
...
mod tests {
    ...
    fn wc_aspect_is_the_decoded_frames_own_ratio() {
        ...
        assert!(
            (build_uniforms(&s, cam, &l, Some((1280, 720)), (16, 16)).wc_aspect - 16.0 / 9.0).abs()
                < 1e-6
        );
        assert_eq!(
            build_uniforms(&s, cam, &l, Some((720, 720)), (16, 16)).wc_aspect,
            1.0
        );
