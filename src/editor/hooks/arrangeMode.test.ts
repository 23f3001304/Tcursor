...
describe("nextArrangeMode", () => {
  ...
  // The event `useArrangeMode.onSel` dispatches on a pill click, which is why re-clicking the
  // already-selected pill re-enters even though `sel` itself never changes - the hook-level
  // regression test for that wiring is `useArrangeMode.test.tsx`.
...
describe("arrangeSeekMs", () => {
  const seg = (over: Partial<LayoutSeg> = {}): LayoutSeg => ({
    id: "s1", start_ms: 1000, end_ms: 3000, layout: "camera", transition_ms: 200, easing: "smooth",
    transition_out_ms: 0, easing_out: "smooth", ...over,
  });

  it("is null while the playhead is already inside the span", () => {
    expect(arrangeSeekMs(seg(), 1000)).toBeNull(); // inclusive start
  ...
  it("lands past the entry transition, on the SETTLED layout rather than mid-fade", () => {
    ...
    expect(arrangeSeekMs(seg(), 3000)).toBe(1200); // end_ms is outside [start, end)
  });
