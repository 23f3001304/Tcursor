...
use crate::settings::appearance::{
    layout_for, overlay_for, AppearanceSettings, CamAspect, CamCorner, CamRing, CamShape,
};
...
fn screen_default_matches_today_inset_and_bubble() {
    ...
    assert!((s.camera.radius - 210.0).abs() < 1e-3);
...
fn camera_default_is_big_centered_rounded_square() {
    ...
    assert!((s.camera.radius - 0.04 * 1920.0).abs() < 1.0);
...
fn corner_and_shape_knobs_apply() {
    ...
    assert_eq!(s.camera.rect.x, 3840.0 - 420.0 - 80.0);
    assert_eq!(s.camera.rect.y, 80.0);
    assert_eq!(s.camera.radius, 0.0);
}
...
fn wide_aspect_widens_bubble_and_keeps_right_anchor() {
    ...
    let expect_w = (420.0f32 * 16.0 / 9.0).round();
    assert_eq!(ov.width_px as f32, expect_w);
    assert_eq!(ov.size_px, 420);
    let s = resolve(
        LayoutId::Screen,
        &layout_for(ma, 3840, 2160),
        &ov,
...
fn cam_ring_flows_onto_camera_panel_only() {
    let mut a = AppearanceSettings::default();
    a.screen.cam_ring = Some(CamRing {
        width: 0.05,
        color: [11, 22, 33],
    });
    let ma = a.for_id(LayoutId::Screen);
    ...
    assert_eq!(s.screen.ring_px, 0.0);
}
#[test]
...
fn shrink_is_identity_at_no_zoom_and_min_at_full() {
    ...
    assert!((full.ring_px - 12.0).abs() < 0.5);
    assert_eq!(full.ring_color, [255, 0, 0]);
}
...
fn empty_track_leaves_static_scene_camera_rect_unchanged() {
    ...
    assert_eq!(
        after.camera.rect, before.camera.rect,
        "static overlay_for rect must be untouched"
    );
}
