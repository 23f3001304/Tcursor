...
3. **The panel's one `Disclosure`**, holding Look and Frame (Accent Colors moved to Settings > Interface on 2026-09-15: an interface colour has no business in a panel about what sits behind the screen). Nothing is removed and nothing is renamed; those three sections are tuning applied to a background you have already chosen, so choosing stays in sight and tuning is one click away and remembered. This was not a preference: the choosing stack plus 215 is well over a 620px slot, so something had to go under it, and the tuning was the honest candidate.
...
**Corner Radius, Padding** - same controls, wired to `doc.settings.appearance.screen.{screen_radius,pad}`; Corner Radius and Padding are the "Frame" group's two-up row, and both groups plus Accent Colors sit under **More**.
...
**Dim** - `0..80%` slider writes `background.dim` (`0..0.8`), a black overlay over whichever background actually has pixels. It sits beside Blur, outside the tab switch, because it applies to a wallpaper, an image and a video alike. Applied exactly once per side: in Rust for everything the backend rasterises, in `stage/canvas/stageBg.ts` for the preview's own video draw.
