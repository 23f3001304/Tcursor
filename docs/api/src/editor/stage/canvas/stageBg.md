# src/editor/stage/canvas/stageBg.ts
...
- `src/editor/stage/canvas/previewCanvas.ts` (`drawPreview`) - calls `drawBackground` once per frame.
- `src/editor/stage/useStageInvalidation.ts` - owns the `StageBgState` and keeps the video in step with the transport.
- `src/editor/hooks/doc/useEditorData.ts` - builds the `StageBg` from `doc.settings.background`.
